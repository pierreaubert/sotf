...
mod tests {
    ...
    use crate::force::{Simulation, SimulationNode, ForceManyBody, ForceCenter};
    ...
    fn test_hierarchy_golden() {
        ...
        assert!(r.y >= a_node.y.min(b_node.y) && r.y <= a_node.y.max(b_node.y)); // Root y is between children
    }
    ...
    fn test_force_simulation_golden() {
        ...
        let mut sim = Simulation::new(vec![n1.clone(), n2.clone()])
            .force(Box::new(ForceManyBody::new())); // Default strength -30 (repulsion)

...
mod tests {
    ...
    fn test_top_down_view_projection() {
        ...
        let center = camera.project_to_screen(Vec3::new(0.0, 0.5, 0.0), width, height).unwrap();
        ...
        let pos_0 = camera.project_to_screen(Vec3::new(1.0, 0.0, 0.0), width, height).unwrap();
        assert!((pos_0.y - 500.0).abs() < 10.0, "Z=0 should be near center Y"); 
        ...
        let pos_n180 = camera.project_to_screen(Vec3::new(1.0, 0.0, -1.0), width, height).unwrap();
        assert!(pos_n180.y < 500.0, "Z=-1 should be above center");
        
        // Z Label for 180 (Z=1).
        // Should be near Y=1000 (Bottom).
        let pos_180 = camera.project_to_screen(Vec3::new(1.0, 0.0, 1.0), width, height).unwrap();
        ...
        println!("Test passed: -180 at {}, 0 at {}, 180 at {}", pos_n180.y, pos_0.y, pos_180.y);
    }
    ...
    fn test_xz_plane_projection() {
        ...
        let s_corners: Vec<Vec3> = corners.iter()
        ...
        assert!((s_corners[0].y - s_corners[1].y).abs() < 1.0, "Top edge should be horizontal");
        // Verify Y coordinates (Bottom edge)
        assert!((s_corners[2].y - s_corners[3].y).abs() < 1.0, "Bottom edge should be horizontal");
        
        // Verify X coordinates (Left edge)
        assert!((s_corners[0].x - s_corners[3].x).abs() < 1.0, "Left edge should be vertical");
        // Verify X coordinates (Right edge)
        assert!((s_corners[1].x - s_corners[2].x).abs() < 1.0, "Right edge should be vertical");
        
        // Verify Relative Positions
        assert!(s_corners[0].x < s_corners[1].x, "Min Freq should be Left of Max Freq");
        assert!(s_corners[0].y < s_corners[3].y, "Min Angle (-180) should be Above Max Angle (180) on screen");
    ...
    fn test_zoom_consistency() {
        ...
        assert!((ratio - 2.0).abs() < 0.1, "Zooming in 2x should double the screen size of objects on plane. Ratio was {}", ratio);
    }
}

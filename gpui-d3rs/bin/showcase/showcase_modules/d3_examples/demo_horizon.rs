...
pub fn render(app: &ShowcaseApp, _cx: &mut Context<ShowcaseApp>) -> Div {
    ...
    let colors = [
        rgb(0xeff3ff), rgb(0xbdd7e7), rgb(0x6baed6), rgb(0x2171b5),
    ];
    ...
    for b in 0..bands {
        ...
        for (i, &v) in data.iter().enumerate() {
            ...
            path_d.push_str(&format!(" L {} {}", x_scale.scale(i as f64), y_scale.scale(y)));
        }
        path_d.push_str(&format!(" L {} {}", x_scale.scale((data.len()-1) as f64), y0));
        path_d.push_str(" Z");
    ...
    div()
        ...
        .p_4()
        .child(
            div().text_lg().font_weight(FontWeight::BOLD).mb_4().child("Horizon Chart (Realtime)"),
        )
        .child(
            div()
                .w(px(width as f32))
                ...
                .child(
                    canvas(
                        move |bounds, _, _| {
                            let parsed: Vec<_> = band_paths.iter().map(|d| {

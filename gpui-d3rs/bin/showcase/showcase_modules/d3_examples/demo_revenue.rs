...
pub fn render(_app: &ShowcaseApp, _cx: &mut Context<ShowcaseApp>) -> Div {
    ...
    for i in 0..n_years {
        ...
        let cassette = if y > 5.0 && y < 25.0 {
            15.0 - (y - 15.0).abs()
        } else {
            0.0
        };
        let cd = if y > 10.0 && y < 35.0 {
            40.0 - (y - 25.0).abs() * 1.5
        } else {
            0.0
        };
        let download = if y > 30.0 && y < 40.0 {
            10.0 - (y - 35.0).abs()
        } else {
            0.0
        };
    ...
    let stack = Stack::new()
        .keys(keys)
        .offset(d3rs::shape::stack::StackOffset::Wiggle)
        .order(d3rs::shape::stack::StackOrder::InsideOut);

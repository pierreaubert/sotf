...
### macOS
Download the DMG, open it and drag SotF to Applications.
For the TUI player, download the binary and run from Terminal.

### Linux
Download the AppImage or tarball. For AppImage: `chmod +x SotF-*.AppImage && ./SotF-*.AppImage`.
For the tarball: extract and run `./SotF` or `./sotf-tui`.
...
\`\`\`bash
codesign --verify --verbose=2 --strict /Volumes/SotF/SotF.app
\`\`\`
EOF

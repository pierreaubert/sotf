//! TUI App adapter for equivalence testing.
//!
//! Adapts the TUI App to the common AppAdapter interface.
...
use sotf_audio_player_tui::app::{
    App as TuiApp, ChannelFilter, InputMode, LibrarySortOrder, LibraryViewMode, Screen,
};
use sotf_audio_player_tui::theme::Theme;

/// Adapter wrapping the TUI App for testing
...
// Conversion helpers for TUI types to normalized IDs
...
fn input_mode_to_id(mode: InputMode) -> InputModeId {
    match mode {
        ...
        InputMode::ShowHelp => InputModeId::Help,
        InputMode::ShowError => InputModeId::Normal, // Map error to normal for comparison
    }
}
...
fn id_to_input_mode(id: InputModeId) -> InputMode {
    match id {
        ...
        InputModeId::Help => InputMode::ShowHelp,
    }
}
...
impl AppAdapter for TuiAdapter {
    fn get_state(&self) -> ComparableState {
        ...
        ComparableState {
            ...
            device_count: 0, // TUI doesn't track devices in the same way
        ...
        match op {
            ...
            Operation::AddSelectedAlbumToQueue => {
                if let Some(album) = self.app.library.albums.get(self.app.selected_album_index) {
                    use sotf_audio_player_tui::app::QueueItem;
            ...
            Operation::MovePluginUp => {
                self.app.move_plugin_up(self.app.selected_plugin_index);
            }
            Operation::MovePluginDown => {
                self.app.move_plugin_down(self.app.selected_plugin_index);
            }
            ...
            // Device management (limited in TUI)
            Operation::SelectNextDevice
            | Operation::SelectPreviousDevice
            | Operation::SelectDevice(_) => {
                // TUI doesn't have device selection in the same way
            }
        }
    }
    ...
    fn reset(&mut self) {
        self.app = TuiApp::new(Theme::default());
    }
}

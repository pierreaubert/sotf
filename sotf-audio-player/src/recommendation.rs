...
pub fn recommend_tracks(
    ...
    log::info!("[Recommendation] Starting generation. Queue size: {}, Target duration: {}s", current_queue_paths.len(), target_duration_secs);
    ...
    if !current_queue_paths.is_empty() {
        ...
        log::info!("[Recommendation] Using last {} tracks from queue as seeds", seed_paths.len());
    } else {
        // Fallback: Use top 5 most played tracks
        let top_tracks = db.get_top_tracks_by_play_count(5)?;
        seed_paths.extend(top_tracks);
        log::info!("[Recommendation] Queue empty. Using top {} most played tracks as seeds", seed_paths.len());
    }
    
    if seed_paths.is_empty() {
        log::warn!("[Recommendation] No seed tracks found (empty library or no history). Cannot recommend.");
        return Ok(Vec::new());
    }
    ...
    for path in &seed_paths {
        if let Some(analysis) = db.get_bliss_analysis(path)? {
            ...
            log::debug!("[Recommendation] Seed track has no bliss analysis: {:?}", path);
        }
    }
    
    log::info!("[Recommendation] Found bliss analysis for {}/{} seed tracks", seed_analyses.len(), seed_paths.len());
    ...
    log::info!("[Recommendation] Found {} total candidates with bliss features", candidates.len());
    ...
    log::info!("[Recommendation] Scored {} candidates", scored_candidates.len());
    ...
    for (path, duration, score) in scored_candidates {
        ...
        log::debug!("[Recommendation] Selected: {:?} (score: {:.4}, duration: {}s)", recommendations.last().unwrap(), score, duration);
    }
    
    log::info!("[Recommendation] Generated {} recommendations (total duration: {}s)", recommendations.len(), current_duration);

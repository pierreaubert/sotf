...
impl<'a, 'b> SpinoramaPage<'a, 'b> {
    ...
    pub fn select_speaker(&mut self, speaker: &str) {
        ...
        self.driver.view.update(self.driver.cx, |view, _window, cx| {
             view.state.update(cx, |state, _cx| {
                 state.app.spinorama_eq_state.selected_speaker = Some(speaker.clone());
                 state.app.spinorama_eq_state.optimization_status = OptimizationStatus::Idle; 
             });
        }).unwrap();
    ...
    pub fn set_target_curve(&mut self, curve: &str) {
        ...
        self.driver.view.update(self.driver.cx, |view, _window, cx| {
             view.state.update(cx, |state, _cx| {
                 state.app.spinorama_eq_state.selected_curve = curve.clone();
             });
        }).unwrap();
        self.driver.cx.run_until_parked();
    }
    ...
    pub fn start_optimization(&mut self) {
        self.driver.view.update(self.driver.cx, |view, _window, cx| {
             view.state.update(cx, |state, _cx| {
                 state.app.spinorama_eq_state.optimization_status = OptimizationStatus::Running;
             });
        }).unwrap();
        self.driver.cx.run_until_parked();
    }
    ...
    pub fn wait_for_optimization_completion(&mut self) -> Result<(), String> {
        ...
        loop {
            ...
            let (status, msg) = self.driver.view.update(self.driver.cx, |view, _window, cx| {
                 ...
                 (state.app.spinorama_eq_state.optimization_status.clone(), 
                  state.app.spinorama_eq_state.error_message.clone())
            }).unwrap();
            ...
            match status {
                 ...
                 OptimizationStatus::Running => {
                     ...
                     self.driver.view.update(self.driver.cx, |view, _window, cx| {
                         view.state.update(cx, |state, _cx| {
                             if state.app.spinorama_eq_state.optimization_status == OptimizationStatus::Running {
                                 state.app.spinorama_eq_state.optimization_status = OptimizationStatus::Completed;
                             }
                         });
                     }).unwrap();
                 },
    ...
    pub fn inject_mock_data(&mut self) {
        self.driver.update_app(|app, _| {
            ...
            app.spinorama_eq_state.selected_speaker = Some("Genelec 8351B".to_string());
            app.spinorama_eq_state.selected_version = "ASR".to_string();
            app.spinorama_eq_state.selected_measurement = "CEA2034".to_string();
            
            // Mock available options to pass validation
            app.spinorama_eq_state.available_measurements = vec!["CEA2034".to_string()];
            ...
            let flat = vec![80.0; freqs.len()];
             ...
             app.spinorama_eq_state.spinorama_curves = sotf_audio_player_gpui::app::types::SpinoramaCurves {
            ...
            app.spinorama_eq_state.step = sotf_audio_player_gpui::app::types::SpinoramaStep::Configure;
        });
        self.driver.cx.run_until_parked();
    }
}

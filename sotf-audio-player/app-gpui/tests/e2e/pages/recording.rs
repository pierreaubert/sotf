...
impl<'a, 'b> RecordingPage<'a, 'b> {
    ...
    pub fn is_all_channels_recorded(&mut self) -> bool {
         self.driver.read_app(|app| {
             app.recording_state.all_channels_recorded()
        })
    }

    pub fn verify_recording_results(&mut self) {
        self.driver.read_app(|app| {
             for recording in &app.recording_state.channel_recordings {
    ...
    pub fn get_available_devices(&mut self) -> (Vec<String>, Vec<String>) {
        self.driver.read_app(|app| {
            let input_names = app.input_devices.iter().map(|d| d.name.clone()).collect();
            let output_names = app.output_devices.iter().map(|d| d.name.clone()).collect();
    ...
    pub fn select_devices(&mut self, device_name: &str) {
        self.driver.update_app(move |app, _| {
            // Select for output
            if let Some(idx) = app.output_devices.iter().position(|d| d.name == device_name) {
                 app.selected_output_device_index = idx;
                 app.current_output_device_name = Some(device_name.to_string());
                 ...
                 if let Some(device) = app.output_devices.get(idx).cloned() {
                     app.recording_state.playback_config.device_name = device.name.clone();
                     app.recording_state.playback_config.device_id = device.device_id.unwrap_or(device.name);
                     // Set max channels
                     if let Some(config) = device.default_config { 
                        app.recording_state.playback_config.num_channels = config.channels as usize;
                        app.recording_state.playback_config.sample_rate = config.sample_rate;
                     }
                 }
            }

            // Select for input
            if let Some(idx) = app.input_devices.iter().position(|d| d.name == device_name) {
                 app.selected_input_device_index = idx;
                 app.current_input_device_name = Some(device_name.to_string());

                 if let Some(device) = app.input_devices.get(idx).cloned() {
                     app.recording_state.recording_config.device_name = device.name.clone();
                     app.recording_state.recording_config.device_id = device.device_id.unwrap_or(device.name);
                     if let Some(config) = device.default_config { 
                        app.recording_state.recording_config.num_channels = config.channels as usize;
                        app.recording_state.recording_config.sample_rate = config.sample_rate;
                     }
                 }
            }
        });
    ...
    pub fn get_device_max_channels(&mut self) -> usize {
        self.driver.read_app(|app| {
             app.recording_state.playback_config.num_channels
        })
    }
    ...
    pub fn set_speaker_config(&mut self, config: SpeakerConfiguration) {
         self.driver.update_app(move |app, _| {
             ...
             app.recording_state.playback_config.channel_mappings = names.iter().enumerate().map(|(i, name)| {
    ...
    pub fn configure_signal(&mut self, signal_type: RecordingSignalType, duration: f32, level_db: f32) {
        self.driver.update_app(move |app, _| {
            ...
            app.recording_state.signal_duration_secs = duration;
            app.recording_state.signal_level_db = level_db;
            
            // If sweep, set reasonable defaults
            if signal_type == RecordingSignalType::Sweep {
                app.recording_state.sweep_start_freq = 100.0;
                app.recording_state.sweep_end_freq = 1000.0; // Short sweep
            }
        });
    }

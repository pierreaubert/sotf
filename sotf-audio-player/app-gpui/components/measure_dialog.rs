...
use gpui::*;
use gpui_ui_kit::{
    Button, ButtonVariant, Dialog, DialogSize, HStack, Select, SelectOption, StackAlign,
...
impl PlayerView {
    pub(crate) fn render_measure_dialog(&self, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let step_content = match measure_state.step {
            MeasureStep::DeviceSelection => self
                .render_measure_device_selection(&measure_state, &theme, cx)
                .into_any_element(),
            MeasureStep::SignalConfig => self
                .render_measure_signal_config(&measure_state, &theme, cx)
                .into_any_element(),
            MeasureStep::Running => self
                .render_measure_running(&measure_state, &theme)
                .into_any_element(),
            MeasureStep::Results => self
                .render_measure_results(&measure_state, &theme)
                .into_any_element(),
        };
    ...
    fn render_measure_device_selection(
        ...
        let out_options: Vec<SelectOption> = (0..max_out)
            .map(|i| SelectOption::new(format!("{}", i), format!("Channel {}", i + 1)))
            .collect();

        let in_options: Vec<SelectOption> = (0..max_in)
            .map(|i| SelectOption::new(format!("{}", i), format!("Channel {}", i + 1)))
            .collect();
            ...
            .child(
                VStack::new()
                    ...
                    .child(
                        HStack::new()
                            .spacing(StackSpacing::Sm)
                            .child(self.render_option_chip(
                                "Sweep",
                                measure_state.signal_type == "sweep",
                                cx,
                                |state, _val| {
                                    if let Some(ms) = &mut state.app.measure_state {
                                        ms.signal_type = "sweep".to_string();
                                    }
                                },
                            ))
                            .child(self.render_option_chip(
                                "Pink Noise",
                                measure_state.signal_type == "pink-noise",
                                cx,
                                |state, _val| {
                                    if let Some(ms) = &mut state.app.measure_state {
                                        ms.signal_type = "pink-noise".to_string();
                                    }
                                },
                            )),
                    ),
                ...
                VStack::new()
                    ...
                    .child(
                        HStack::new()
                            ...
                            .child(self.render_option_chip(
                                "2.0s",
                                measure_state.duration == "2.0",
                                cx,
                                |state, _val| {
                                    if let Some(ms) = &mut state.app.measure_state {
                                        ms.duration = "2.0".to_string();
                                    }
                                },
                            ))
                            .child(self.render_option_chip(
                                "5.0s",
                                measure_state.duration == "5.0",
                                cx,
                                |state, _val| {
                                    if let Some(ms) = &mut state.app.measure_state {
                                        ms.duration = "5.0".to_string();
                                    }
                                },
                            ))
                            .child(self.render_option_chip(
                                "10.0s",
                                measure_state.duration == "10.0",
                                cx,
                                |state, _val| {
                                    if let Some(ms) = &mut state.app.measure_state {
                                        ms.duration = "10.0".to_string();
                                    }
                                },
                            )),
                    ),
                ...
                VStack::new()
                    ...
                    .child(
                        HStack::new()
                            ...
                            .child(self.render_option_chip(
                                "-10 dB",
                                (measure_state.level - (-10.0)).abs() < 0.1,
                                cx,
                                |state, _val| {
                                    if let Some(ms) = &mut state.app.measure_state {
                                        ms.level = -10.0;
                            ...
                            .child(self.render_option_chip(
                                "-20 dB",
                                (measure_state.level - (-20.0)).abs() < 0.1,
                                cx,
                                |state, _val| {
                                    if let Some(ms) = &mut state.app.measure_state {
                                        ms.level = -20.0;
                            ...
                            .child(self.render_option_chip(
                                "-40 dB",
                                (measure_state.level - (-40.0)).abs() < 0.1,
                                cx,
                                |state, _val| {
                                    if let Some(ms) = &mut state.app.measure_state {
                                        ms.level = -40.0;
            ...
            div()
                ...
                .child(
                    VStack::new()
                        ...
                        .child(
                            Text::new("Measurement Complete")
                                .weight(TextWeight::Bold)
                                .size(TextSize::Lg),
                        )
                        .child(
                            Text::new(format!("Captured {} points.", res.frequencies.len()))
                                .muted(true),
                        )
                        .child(
                            div()
    ...
    fn render_measurement_preview(
        &self,
        result: &crate::app::types::MeasurementResult,
        theme: &crate::theme::Theme,
    ) -> impl IntoElement {
        let width = 600.0;
        let height = 250.0;
        ...
        let points: Vec<LinePoint> = result
            ...
            .map(|(&f, &db)| LinePoint::new(f as f64, db as f64))
            .collect();
    ...
    fn render_measure_footer(
        ...
        HStack::new()
            ...
            .child(match measure_state.step {
                MeasureStep::DeviceSelection => Button::new("meas_next_1", "Next")
    ...
    pub fn start_measurement_task(&self, cx: &mut Context<Self>) {
        ...
        let (signal_str, duration, out_ch, in_ch, out_dev, in_dev, sample_rate) =
            if let Some(ms) = &state.app.measure_state {
                let out_idx = state.app.selected_output_device_index;
                let sr = state
                    .app
                    .output_devices

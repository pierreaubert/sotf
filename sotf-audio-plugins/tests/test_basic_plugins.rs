...
use sotf_plugins::{
    GainPlugin, DelayPlugin, MatrixPlugin, InPlacePlugin, InPlacePluginAdapter, 
    Plugin, ProcessContext, PluginHost
};
...
fn test_delay_plugin() {
    ...
    host.add_plugin(Box::new(InPlacePluginAdapter::new(delay))).unwrap();
    ...
    assert!((output[ch_idx] - 1.0).abs() < 0.1, "Expected impulse at sample {}, got {}", delay_samples, output[ch_idx]);
}

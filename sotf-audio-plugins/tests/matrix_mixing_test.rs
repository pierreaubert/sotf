...
fn test_mono_summing_loudness_correlated() {
    ...
    let mut plugin = MatrixPlugin::with_matrix(input_channels, output_channels, matrix_data).unwrap();

    let context = ProcessContext { sample_rate: 44100, num_frames: 64 };
    ...
    for sample in output.iter() {
        assert!((sample - 2.0_f32).abs() < 1e-6, "Simple sum 1+1 should be 2.0");
    }
    ...
    for sample in output.iter() {
        assert!((sample - 1.0_f32).abs() < 1e-6, "Normalized sum 0.5+0.5 should be 1.0");
    }
}
...
fn test_mid_side_encoding_decoding() {
    ...
    for i in 0..num_frames {
        ...
        assert!((side - 1.0_f32).abs() < 1e-6, "Left Only: Side should be 1.0");
    }
    ...
    for i in 0..num_frames {
        let mid = output[i*2];
        let side = output[i*2+1];
        assert!((mid - 1.0_f32).abs() < 1e-6, "Right Only: Mid should be 1.0");
        assert!((side - -1.0_f32).abs() < 1e-6, "Right Only: Side should be -1.0");
    }
}
...
fn test_mid_side_roundtrip() {
    ...
    let mut encoder = MatrixPlugin::with_matrix(2, 2, vec![
        1.0, 1.0,
        1.0, -1.0
    ]).unwrap();
    
    let mut decoder = MatrixPlugin::with_matrix(2, 2, vec![
        0.5, 0.5,
        0.5, -0.5
    ]).unwrap();
...
fn test_mono_mix_down_laws() {
    ...
    let mut input_corr = Vec::with_capacity(num_frames * 2);
    for _ in 0..num_frames { input_corr.push(1.0); input_corr.push(1.0); }
    
    plugin_3db.process(&input_corr, &mut output_3db, &context).unwrap();
    plugin_6db.process(&input_corr, &mut output_6db, &context).unwrap();
    ...
    for _ in 0..num_frames { input_panned.push(1.0); input_panned.push(0.0); }
    
    plugin_3db.process(&input_panned, &mut output_3db, &context).unwrap();
    plugin_6db.process(&input_panned, &mut output_6db, &context).unwrap();

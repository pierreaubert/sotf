...
impl Plugin for PndPlugin {
    ...
    fn parameters(&self) -> Vec<Parameter> {
        vec![
            Parameter::new_float("correction_strength", "Correction Strength", CORRECTION_STRENGTH_DEFAULT, CORRECTION_STRENGTH_MIN, CORRECTION_STRENGTH_MAX),
            Parameter::new_float("analysis_window_ms", "Analysis Window (ms)", ANALYSIS_WINDOW_MS_DEFAULT, ANALYSIS_WINDOW_MS_MIN, ANALYSIS_WINDOW_MS_MAX),
            Parameter::new_float("drift_smoothing", "Drift Smoothing", DRIFT_SMOOTHING_DEFAULT, DRIFT_SMOOTHING_MIN, DRIFT_SMOOTHING_MAX),
        ]
    }
    ...
    fn process(&mut self, input: &[f32], output: &mut [f32], context: &ProcessContext) -> PluginResult<()> {
        ...
        let drift_ratio = if let Some(analyzer) = &mut self.analyzer {
            ...
            let mono_input: Vec<f32> = input.chunks(self.channels)
                .map(|chunk| chunk[0])
                .collect();
        ...
        if let Some(resampler) = &mut self.resampler {
            ...
            resampler.set_resample_ratio(final_ratio, true).map_err(|e| format!("{:?}", e))?;
            ...
            let planar_output = resampler.process(&planar_input, None).map_err(|e| format!("{:?}", e))?;

        ...
        for frame in buffer.chunks_mut(self.channels) {
            for (ch, sample) in frame.iter_mut().enumerate() {
                ...
                if processed_abs > self.envelope[ch] {
                    ...
                    // Decay
                    self.envelope[ch] =
                        self.envelope[ch] * self.decay + processed_abs * (1.0 - self.decay);
                }
...
mod tests {
    ...
    fn test_transient_suppression() {
        ...
        assert!(
            processed_spike < original_spike,
            "Spike was not attenuated: {} -> {}",
            original_spike,
            processed_spike
        );
        ...
        assert!(
            processed_spike > 0.1,
            "Spike was attenuated too much: {}",
            processed_spike
        );
        assert!(
            processed_spike < 1.0,
            "Spike was not attenuated enough: {}",
            processed_spike
        );

        // Steady state should be unaffected
        assert!(
            (buffer[0] - original_first).abs() < 1e-6,
            "First sample affected: {} vs {}",
            buffer[0],
            original_first
        );
    }

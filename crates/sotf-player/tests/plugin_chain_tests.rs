...
#[test]
fn binaural_channel_reconstruction_preserves_all_non_channel_settings() {
    let mut binaural = PluginSettings::default_for(&PluginType::BinauralDecoder).unwrap();
    ...
    *sofa_file = "custom-listener.sofa".into();
    *input_channels = 2;
    *externalization = 0.81;
    *near_field_strength = 0.42;
    *crossfade_mode = 2;
    *late_reverb_enabled = true;
    *late_reverb_mix = 0.37;
    *late_reverb_rt60 = 1.73;
    *late_reverb_damping = 0.29;
    *crossfade_ms = 47.5;
    *head_yaw_deg = 23.0;
    *head_pitch_deg = -11.0;
    *head_roll_deg = 7.0;
    *hrtf_database_dir = "/hrtf/custom".into();
    *head_width_cm = 16.9;
    *ear_height_cm = 12.4;
    ...
    graph
        ...
        .unwrap();
    let binaural_index = graph.user_plugin_insert_index();
    graph
        .insert_plugin_settings(binaural_index, binaural)
        .unwrap();
    ...
    assert_eq!(sofa_file, "custom-listener.sofa");
    assert_eq!(*externalization, 0.81);
    assert_eq!(*near_field_strength, 0.42);
    assert_eq!(*crossfade_mode, 2);
    assert!(*late_reverb_enabled);
    assert_eq!(*late_reverb_mix, 0.37);
    assert_eq!(*late_reverb_rt60, 1.73);
    assert_eq!(*late_reverb_damping, 0.29);
    assert_eq!(*crossfade_ms, 47.5);
    assert_eq!(*head_yaw_deg, 23.0);
    assert_eq!(*head_pitch_deg, -11.0);
    assert_eq!(*head_roll_deg, 7.0);
    assert_eq!(hrtf_database_dir, "/hrtf/custom");
    assert_eq!(*head_width_cm, 16.9);
    assert_eq!(*ear_height_cm, 12.4);
}

...
mod tests {
    ...
    mod property_tests {
        use super::super::api_parse_range_header;
        use proptest::prelude::*;

        proptest! {
            #![proptest_config(ProptestConfig { cases: 256, ..ProptestConfig::default() })]

            /// INVARIANT: a missing or non-bytes Range header always returns Ok(None) or Err(()).
            #[test]
            fn parse_range_header_rejects_invalid_prefix(
                header in prop::option::of("[^b][^=]*"),
                file_len in 0u64..1_000_000u64,
            ) {
                let result = api_parse_range_header(header.as_deref(), file_len);
                if header.as_deref().map(|h| h.trim().starts_with("bytes=")).unwrap_or(false) {
                    // Valid prefix branch is covered by other tests; do not assert here.
                } else {
                    prop_assert!(
                        result == Ok(None) || result == Err(()),
                        "non-bytes/missing header should be None or Err: got {:?}",
                        result
                    );
                }
            }

            /// INVARIANT: a single closed byte range is parsed consistently with
            /// the HTTP Range spec: both bounds are inclusive and clamped to the file.
            #[test]
            fn parse_range_header_single_closed_range(
                start in 0u64..1_000_000u64,
                end in 0u64..1_000_000u64,
                file_len in 1u64..1_000_000u64,
            ) {
                let header = format!("bytes={}-{}", start, end);
                let result = api_parse_range_header(Some(&header), file_len);
                if end < start {
                    prop_assert_eq!(result, Err(()));
                } else if start >= file_len {
                    prop_assert_eq!(result, Err(()));
                } else {
                    let expected_end = end.min(file_len - 1);
                    prop_assert_eq!(result, Ok(Some((start, expected_end))));
                }
            }
            ...
            #[test]
            fn parse_range_header_rejects_multi_range_and_zero_suffix(
                a in 0u64..100u64,
                b in 0u64..100u64,
                file_len in 1u64..1_000_000u64,
            ) {
                let multi = format!("bytes={}-{},{}-{}", a, b, b + 1, b + 2);
                prop_assert_eq!(api_parse_range_header(Some(&multi), file_len), Err(()));
                prop_assert_eq!(api_parse_range_header(Some("bytes=-0"), file_len), Err(()));
            }
        }
    }
}

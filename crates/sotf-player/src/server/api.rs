...
pub(super) fn api_library_albums_json(state: &Arc<ServerState>, path: &str) -> Result<Value, String> {
...
pub(super) fn api_library_album_tracks_json(state: &Arc<ServerState>, route: &str) -> Result<Value, String> {
    let album_id = route
...
pub(super) fn api_add_album_to_queue(state: &Arc<ServerState>, request: &ApiRequest) -> Result<Value, String> {
...
pub(super) fn api_delete_queue_album(state: &Arc<ServerState>, request: &ApiRequest) -> Result<Value, String> {
...
pub(super) fn api_jump_queue_album(state: &Arc<ServerState>, request: &ApiRequest) -> Result<Value, String> {
    let index = api_body_index(request)?;
    let mut queue = state.queue.lock();
    let source = queue
        .jump_to(index)
...
pub(super) fn api_pairing_complete(state: &Arc<ServerState>, request: &ApiRequest) -> Result<Value, String> {
    if !state
        .pairing_mode
        .load(std::sync::atomic::Ordering::Relaxed)
    {

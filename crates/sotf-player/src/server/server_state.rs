...
pub(super) fn insert_live_trusted_client(state: &ServerState, fingerprint: &str) -> Result<(), String> {
...
pub(super) fn remove_live_trusted_client(state: &ServerState, fingerprint: &str) -> Result<(), String> {
    let mut trusted = state
        .trusted_client_fingerprints
        .lock()
        .map_err(|e| format!("trusted fingerprint lock poisoned: {e}"))?;

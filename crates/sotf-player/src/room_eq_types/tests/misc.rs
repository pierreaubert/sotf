...
use crate::recording_types::{
    ChannelRecording, ChannelRecordingState, CtcMatrixExportStrategy, DelayProbeChannelResult,
    DelayProbeResults, RecordingResult,
};
use std::path::Path;
...
pub(super) fn assert_room_eq_matrix_nodes_have_width(
    ...
    for node in graph.nodes.iter().filter(|node| {
        ...
        let plugin =
            sotf_plugins::create_plugin("matrix", &node.parameters, node.input_channels, 48_000)

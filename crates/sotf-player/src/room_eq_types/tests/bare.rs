...
use super::super::*;
use autoeq::roomeq::{
    BassManagementReport, BassManagementRoute, BassManagementRoutingGraph, OptimizationMetadata,
    PluginConfigWrapper,
};
...
fn test_build_room_eq_graph_emits_driver_branches_before_channel_plugins() {
    ...
    assert!(
        graph
            .edges
            .iter()
            .any(|edge| { edge.from_node == tweeter_highpass_id && edge.to_node == driver_sum_id })
    );
    assert!(
        graph
            .edges

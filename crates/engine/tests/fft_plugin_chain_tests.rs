...
use sotf_audio::plugins::{PluginChain, PluginType, PluginSettings};
...
fn test_fft_chain_arbitrary_frame_sizes() {
    ...
    assert!(state.last_error.is_none(), "Engine reported error: {:?}", state.last_error);
    ...
    let spectrum_data_any = engine.get_cached_plugin_data(2)
        .expect("Failed to get spectrum data");
    
    let spectrum_data = spectrum_data_any.downcast_ref::<sotf_audio::SpectrumData>()
        .expect("Failed to downcast spectrum data");
    
    // Verify we have a peak around 1000Hz
    assert!(spectrum_data.peak_magnitude > -50.0, "Spectrum peak too low: {}dB", spectrum_data.peak_magnitude);
    
    let (peak_bin_idx, _) = spectrum_data.magnitudes.iter().enumerate()
        .max_by(|a, b| a.1.partial_cmp(b.1).unwrap())
        .unwrap();
    
    let peak_freq = spectrum_data.frequencies[peak_bin_idx];
    assert!((peak_freq - 1000.0).abs() < 200.0, "Peak frequency mismatch: {}Hz (expected ~1000Hz)", peak_freq);
}

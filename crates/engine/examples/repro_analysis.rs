...
use sotf_audio::signal_analysis::{analyze_recording, WavAnalysisConfig};
...
fn main() {
    ...
    if !result.harmonic_distortion_db.is_empty() {
         println!("Distortion[0] len: {}", result.harmonic_distortion_db[0].len());
    }
    ...
    println!("Clarity C50 val[0]: {}", result.clarity_c50_db.first().unwrap_or(&-1.0));

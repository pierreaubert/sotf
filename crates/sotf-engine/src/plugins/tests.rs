...
#[test]
fn compressor_defaults_emit_only_supported_runtime_parameters() {
    ...
    for legacy in [
        ...
        assert!(
            config.parameters.get(legacy).is_none(),
            "unsupported {legacy} leaked into Compressor runtime parameters"
        );
    }
    sotf_plugins::create_plugin(&config.plugin_type, &config.parameters, 2, 48_000)
        .expect("default Compressor settings must construct the DSP plugin");
}

#[test]
fn band_split_defaults_keep_channel_topology_out_of_runtime_parameters() {
    let settings = PluginSettings::default_for(&PluginType::BandSplit).unwrap();
    let config = settings.to_plugin_config(48_000.0);

    assert!(config.parameters.get("channels").is_none());
    sotf_plugins::create_plugin(&config.plugin_type, &config.parameters, 2, 48_000)
        .expect("default Band Split settings must construct the DSP plugin");
}

#[test]
fn band_merge_defaults_keep_channel_topology_out_of_runtime_parameters() {
    let settings = PluginSettings::default_for(&PluginType::BandMerge).unwrap();
    let config = settings.to_plugin_config(48_000.0);

    assert!(config.parameters.get("channels").is_none());
    sotf_plugins::create_plugin(&config.plugin_type, &config.parameters, 4, 48_000)
        .expect("default Band Merge settings must construct the DSP plugin");
}

#[test]
fn dither_settings_preserve_defaults_and_engine_config() {
    assert!(PluginType::all().contains(&PluginType::Dither));
    assert_eq!(
        PluginType::from_wire_name("dither"),
        Some(PluginType::Dither)
    );

    let defaults = PluginSettings::default_for(&PluginType::Dither).unwrap();
    assert!(matches!(
        defaults,
        PluginSettings::Dither {
            bit_depth: 0,
            noise_shaping: true,
            dither_type: 0,
        }
    ));
    assert_eq!(defaults.plugin_type(), PluginType::Dither);
    ...
    let restored: PluginSettings = serde_json::from_value(serialized.clone()).unwrap();
    assert_eq!(serde_json::to_value(&restored).unwrap(), serialized);

    let config = restored.to_plugin_config(48_000.0);
    assert_eq!(config.plugin_type, "dither");
    assert_eq!(config.parameters["bit_depth"], 2);
    assert_eq!(config.parameters["noise_shaping"], false);
    assert_eq!(config.parameters["dither_type"], 2);
    sotf_plugins::create_plugin(&config.plugin_type, &config.parameters, 2, 48_000)
        .expect("Dither engine settings must construct the DSP plugin");
}

...
impl Hdf5File {
    ...
    pub(super) fn parse_oh_v1_continuation(&self, addr: u64, _len: u64, group: &mut GroupInfo) -> Result<()> {
    ...
    pub(super) fn parse_symbol_table_msg(&self, c: &mut Cursor<'_>, group: &mut GroupInfo) -> Result<()> {
    ...
    pub(super) fn parse_btree_v1(&self, addr: u64, heap_data: &[u8], group: &mut GroupInfo) -> Result<()> {
        let mut c = self.cursor_at(addr);
        let sig = c.bytes(4)?;
        if sig != b"TREE" {
            return Err(SofaError::InvalidStructure(
...
impl Hdf5File {
    ...
    pub(super) fn parse_btree_v2(&self, addr: u64, fh: &FractalHeapInfo) -> Result<Vec<HeapRecord>> {
        let mut c = self.cursor_at(addr);
        let sig = c.bytes(4)?;
        if sig != b"BTHD" {
    ...
    pub(super) fn expected_dataset_bytes(&self, ds: &DatasetInfo, elem_size: usize) -> Result<usize> {

...
use crate::app::{
    ConfigureSubScreen, FocusedPane, HeadphoneEqStep, InputMode, Screen, SpinoramaStep,
};
...
fn tab_cycles_through_all_screens() {
    let mut app = app_on_library();
    assert_eq!(app.current_screen, Screen::Library);
    assert_eq!(app.focused_pane, FocusedPane::Main);
...
#[test]
fn tab_from_configure_goes_to_meters() {
    let mut app = app_on_library();
    app.current_screen = Screen::Configure;
    app.configure_tab_focused = true;

    // Tab on Configure with tab_focused: configure handler cycles tabs
    // This advances configure_sub_screen, not the main screen
    send_keys(&mut app, &[KeyCode::Tab]);
    // configure::handle_configure_keys: Tab when tab_focused → next sub-screen
    assert_eq!(app.current_screen, Screen::Configure);
...
#[test]
fn esc_from_meters_goes_to_main() {
    let mut app = app_on_library();
    app.focused_pane = FocusedPane::Meters;
...
fn configure_digit_keys_select_tabs() {
    ...
    assert!(!app.configure_tab_focused); // enters content
...
#[test]
fn configure_enter_unfocuses_tab_bar() {
    let mut app = app_on_library();
    app.current_screen = Screen::Configure;
    app.configure_tab_focused = true;
    app.configure_sub_screen = ConfigureSubScreen::RoomEq;
...
#[test]
fn spinorama_configure_tab_advances_to_optimize() {
    let mut app = app_on_spinorama_select();
...
#[test]
fn spinorama_configure_enter_advances_to_optimize() {
    let mut app = app_on_spinorama_select();
    app.spinorama_eq.step = SpinoramaStep::Configure;
...
#[test]
fn spinorama_results_tab_advances_to_update_plugin() {
    let mut app = app_on_spinorama_select();
...
// ── 6b. Spinorama wizard Left arrow goes back ────────────────────────────────

#[test]
fn spinorama_optimize_left_goes_to_configure() {
    let mut app = app_on_spinorama_select();
    app.spinorama_eq.step = SpinoramaStep::Optimize;
...
#[test]
fn spinorama_results_left_goes_to_optimize() {
    let mut app = app_on_spinorama_select();
    app.spinorama_eq.step = SpinoramaStep::Results;
...
#[test]
fn spinorama_update_plugin_left_goes_to_results() {
    let mut app = app_on_spinorama_select();
    app.spinorama_eq.step = SpinoramaStep::UpdatePlugin;
...
#[test]
fn spinorama_left_chain_from_update_to_configure() {
    let mut app = app_on_spinorama_select();
    app.spinorama_eq.step = SpinoramaStep::UpdatePlugin;
...
// ── 7. Left arrow goes back in all wizards ───────────────────────────────────
//
// For steps where Left is NOT used for field adjustment, it should go back to
// the previous step (same as BackTab).  Configure/Config steps use Left for
// value editing so they are excluded.

// ── 7a. Room EQ ──

#[test]
fn room_eq_tab_advances_from_load_to_configure() {
...
#[test]
fn room_eq_optimize_left_goes_to_configure() {
    let mut app = app_on_room_eq();
...
#[test]
fn room_eq_review_left_goes_to_optimize() {
    let mut app = app_on_room_eq();
...
#[test]
fn room_eq_export_left_goes_to_review() {
    let mut app = app_on_room_eq();
...
#[test]
fn headphone_eq_optimize_left_goes_to_configure() {
...
#[test]
fn headphone_eq_results_left_goes_to_optimize() {
    let mut app = app_on_headphone_eq();
...
#[test]
fn headphone_eq_update_plugin_left_goes_to_results() {
    let mut app = app_on_headphone_eq();
...
#[test]
fn recording_capture_left_goes_to_config() {
...
#[test]
fn recording_evaluating_left_goes_to_capture() {
    let mut app = app_on_recording();
...
#[test]
fn recording_saving_left_goes_to_evaluating() {
    let mut app = app_on_recording();
...
#[test]
fn sequence_c_5_enter_enter_reaches_spinorama_optimize() {
    let mut app = app_on_library();
    app.spinorama_eq.available_speakers = vec!["Test Speaker".to_string()];
    app.spinorama_eq.update_filter();
    ...
    send_keys(
        ...
        &[
            ...
            KeyCode::Enter,     // → Configure → Optimize step
        ],
    );
    assert_eq!(app.current_screen, Screen::Configure);
    assert_eq!(app.configure_sub_screen, ConfigureSubScreen::SpinoramaEq);
...
#[test]
fn sequence_c_5_enter_enter_tab_reaches_spinorama_results() {
    let mut app = app_on_library();
    app.spinorama_eq.available_speakers = vec!["Test Speaker".to_string()];
    app.spinorama_eq.update_filter();
    ...
    send_keys(
        ...
        &[
            ...
            KeyCode::Enter,     // → select speaker → Configure step
            KeyCode::Enter,     // → Configure → Optimize step
            KeyCode::Tab,       // → Optimize → Results step
        ],
    );
    assert_eq!(app.current_screen, Screen::Configure);
    assert_eq!(app.configure_sub_screen, ConfigureSubScreen::SpinoramaEq);
...
fn sequence_spinorama_full_forward_and_back() {
    ...
    // Forward: Select → Configure → Optimize → Results → UpdatePlugin
    send_keys(
        &mut app,
        &[
            KeyCode::Char('C'),
            ...
            KeyCode::Tab,   // Configure → Optimize
            KeyCode::Tab,   // Optimize → Results
            KeyCode::Tab,   // Results → UpdatePlugin
        ],
    );
    assert_eq!(app.spinorama_eq.step, SpinoramaStep::UpdatePlugin);
...
#[test]
fn shift_m_focuses_meters() {
    let mut app = app_on_library();
    assert_eq!(app.focused_pane, FocusedPane::Main);

    handle_key_event(&mut app, shift_key(KeyCode::Char('M')));
    assert_eq!(app.focused_pane, FocusedPane::Meters);
}
...
fn channel_conflict_enter_on_suspend_returns_play() {
    ...
    assert_eq!(app.input_mode, InputMode::Normal);
    assert!(matches!(cmd, Some(PlayerCommand::Play(_))));

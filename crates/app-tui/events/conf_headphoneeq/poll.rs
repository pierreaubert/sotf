use super::consts::HEADPHONE_DOWNLOAD_RESULT;
use super::consts::HEADPHONE_LIST_RESULT;
use super::consts::HEADPHONE_OPT_PROGRESS;
use super::consts::HEADPHONE_OPT_RESULT;
use crate::app::App;
use std::sync::{Arc, Mutex};

pub fn poll_headphone_eq_optimization(app: &mut App) -> bool {
    use sotf_audio_player::headphone_eq_types::HeadphoneEqBiquad;
    use sotf_audio_player::room_eq_types::OptimizationStatus;

    if app.headphone_eq.model.optimization_status != OptimizationStatus::Running {
        return false;
    }

    let result_slot = HEADPHONE_OPT_RESULT
        .get_or_init(|| Arc::new(Mutex::new(None)))
        .clone();
    let progress_slot = HEADPHONE_OPT_PROGRESS
        .get_or_init(|| Arc::new(Mutex::new(None)))
        .clone();

    if let Ok(mut guard) = result_slot.lock()
        && let Some(result) = guard.take()
    {
        match result {
            Ok(r) => {
                app.headphone_eq.model.pre_loss = r.initial_loss;
                app.headphone_eq.model.post_loss = r.final_loss;
                app.headphone_eq.model.filters = r
                    .biquads
                    .into_iter()
                    .map(|b| HeadphoneEqBiquad {
                        filter_type: b.filter_type.long_name().to_string(),
                        freq: b.freq,
                        q: b.q,
                        db_gain: b.db_gain,
                    })
                    .collect();
                app.headphone_eq.model.curve_frequencies = r.frequencies.clone();
                app.headphone_eq.model.curve_input = r.input_curve.clone();
                app.headphone_eq.model.curve_target = r.target_curve.clone();
                app.headphone_eq.model.curve_corrected = r.corrected_curve.clone();
                app.headphone_eq.model.curve_filter_response = r.filter_response.clone();
                // Keep the progress-based loss_history; only override if empty
                if app.headphone_eq.model.progress_history.is_empty() {
                    app.headphone_eq.model.progress_history = r.optimization_history.clone();
                }
                app.headphone_eq.model.optimization_status = OptimizationStatus::Completed;
                app.headphone_eq.model.progress = 1.0;
                // Auto-advance to Results
                app.headphone_eq.step = crate::app::HeadphoneEqStep::Results;
            }
            Err(e) => {
                app.headphone_eq.model.optimization_status = OptimizationStatus::Failed;
                app.headphone_eq.model.error_message = Some(e);
            }
        }
        return true;
    }

    if let Ok(mut guard) = progress_slot.lock()
        && let Some((iter, max_iter, loss, pct)) = guard.take()
    {
        app.headphone_eq.model.current_iteration = iter;
        app.headphone_eq.opt_max_iter = max_iter;
        app.headphone_eq.model.current_loss = loss;
        app.headphone_eq.model.progress = pct;
        app.headphone_eq.model.progress_history.push((iter, loss));
        return true;
    }

    false
}

/// Poll headphone list fetch. Returns true if UI needs a redraw.
pub fn poll_headphone_list_load(app: &mut App) -> bool {
    if !app.headphone_eq.model.loading_headphones {
        return false;
    }
    let result_slot = HEADPHONE_LIST_RESULT
        .get_or_init(|| Arc::new(Mutex::new(None)))
        .clone();
    if let Ok(mut guard) = result_slot.lock()
        && let Some(result) = guard.take()
    {
        app.headphone_eq.model.loading_headphones = false;
        match result {
            Ok(headphones) => {
                app.headphone_eq.model.available_headphones = headphones;
                app.headphone_eq.update_filter();
            }
            Err(e) => {
                app.headphone_eq.headphones_error = Some(e);
            }
        }
        return true;
    }
    false
}

/// Poll headphone download. Returns true if UI needs a redraw.
pub fn poll_headphone_download(app: &mut App) -> bool {
    if !app.headphone_eq.model.loading_download {
        return false;
    }
    let result_slot = HEADPHONE_DOWNLOAD_RESULT
        .get_or_init(|| Arc::new(Mutex::new(None)))
        .clone();
    if let Ok(mut guard) = result_slot.lock()
        && let Some(result) = guard.take()
    {
        app.headphone_eq.model.loading_download = false;
        match result {
            Ok(csv_path) => {
                app.headphone_eq.model.measurement_path = csv_path;
            }
            Err(e) => {
                app.headphone_eq.headphones_error = Some(e);
            }
        }
        return true;
    }
    false
}

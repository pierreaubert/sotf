...
fn test_plugin_parameter_sync() {
    ...
    for plugin_type in plugin_types {
        ...
        for i in 0..descriptors.len() {
            ...
            // Name should match
            assert_eq!(desc.name, param.name, "Name mismatch at index {} for {:?}", i, plugin_type);
...
fn test_upmixer_parameter_sync_deep() {
    ...
    // Upmixer is expected to have 38 parameters currently (0-37)
...
fn test_eq_filter_limit() {
    ...
    if let PluginSettings::EQ { filters, max_filters, .. } = &settings {

...
fn draw_api_section(f: &mut Frame, area: Rect, app: &App) {
    ...
    let token_summary = api
        ...
        .unwrap_or_else(|| crate::tui_text!(app, "(auto on enable)"));
    ...
    lines.push(Line::from(Span::styled(
        crate::tui_text!(app, "  Remote apps use this API port."),
        Style::default().fg(app.theme.fg_secondary),
    )));
    lines.push(Line::from(Span::styled(
        crate::tui_text!(app, "  MPD clients use the MPD port."),
        Style::default().fg(app.theme.fg_secondary),
    )));
    ...
    let para = Paragraph::new(lines)
        .block(
            Block::default()
                ...
                .title(crate::tui_text!(app, " SOTF API ")),
        )
...
fn draw_mpd_section(f: &mut Frame, area: Rect, app: &App) {
    ...
    let fields: Vec<(&str, String, bool)> = vec![
        ...
        (
            "Auth Mode",
            match mpd.auth_mode {
                MpdAuthMode::Certificate => crate::tui_text!(app, "Certificate"),
                MpdAuthMode::Password => crate::tui_text!(app, "Password"),
            },
            true,
        ),
        (
            "Password",
            mpd.password.as_ref().map_or_else(
                || crate::tui_text!(app, "(none)"),
                |p| "*".repeat(p.len().min(8)),
            ),
            false,
        ),
        (
            "Trusted Clients",
            crate::i18n::TuiTranslations::for_language(app.ui.language).dynamic_or_verbatim(
    ...
    if invalid_fingerprints > 0 {
        ...
        lines.push(Line::from(Span::styled(
            crate::tui_text!(app, "    Use client certificate SHA-256 fingerprints."),
            Style::default().fg(app.theme.fg_secondary),
        )));
    } else if mpd.enabled
        && mpd.tls_enabled
        ...
        && mpd.trusted_client_fingerprints.is_empty()
    {
        ...
        lines.push(Line::from(Span::styled(
            crate::tui_text!(app, "    fingerprint, or switch Auth Mode to Password."),
            Style::default().fg(app.theme.fg_secondary),
        )));
    } else if mpd.enabled && mpd.auth_mode == MpdAuthMode::Password && mpd.password.is_none() {
        lines.push(Line::from(""));
        lines.push(Line::from(Span::styled(
            crate::tui_text!(app, "  ! Password auth needs a non-empty password."),
            Style::default()
                .fg(app.theme.accent_warning)
                .add_modifier(Modifier::BOLD),
        )));
    ...
    if mpd.auth_mode == MpdAuthMode::Certificate {
        lines.push(Line::from(""));
        lines.push(Line::from(Span::styled(
            crate::tui_text!(app, "  Pairing clients can add trust automatically."),
            Style::default().fg(app.theme.fg_secondary),
        )));
        lines.push(Line::from(Span::styled(
            crate::tui_text!(
    ...
    if let Some(ref fp) = state.tls_fingerprint {
        lines.push(Line::from(""));
        lines.push(Line::from(Span::styled(
            crate::tui_text!(app, format!("  Fingerprint: {}", &fp[..fp.len().min(23)])),
            Style::default().fg(app.theme.fg_secondary),
        )));
    }
    ...
    let para = Paragraph::new(lines)
        .block(
            Block::default()
                ...
                .border_type(border_type)
                .border_style(Style::default().fg(border_color))
                .title(crate::tui_text!(app, " MPD Server ")),
        )
        .wrap(Wrap { trim: false });
    f.render_widget(para, area);
}
...
fn draw_dlna_section(f: &mut Frame, area: Rect, app: &App) {
    ...
    note_lines.push(Line::from(Span::styled(
        crate::tui_text!(app, "  Bind 0.0.0.0 listens on all interfaces."),
        Style::default().fg(app.theme.fg_secondary),
    )));
    note_lines.push(Line::from(""));
    note_lines.push(Line::from(Span::styled(
        crate::tui_text!(app, "  (DLNA uses plain HTTP for"),
        Style::default().fg(app.theme.fg_secondary),
    )));
    note_lines.push(Line::from(Span::styled(
        crate::tui_text!(app, "   device compatibility)"),
        Style::default().fg(app.theme.fg_secondary),
    )));
    ...
    let para = Paragraph::new(note_lines)
        .block(
            Block::default()
                ...
                .border_type(border_type)
                .border_style(Style::default().fg(border_color))
                .title(crate::tui_text!(app, " DLNA Server ")),
        )
        .wrap(Wrap { trim: false });
    f.render_widget(para, area);
}
...
fn render_field_lines<'a>(
    ...
    fields
        ...
        .map(|(i, (label, value, _is_toggle))| {
            ...
            let label = crate::tui_text!(app, *label);

//! Interactive Scatter Chart Debug - Demonstrates legend position and graph ratio controls.
//!
...
fn main() {
    MiniApp::run(
        MiniAppConfig::new("Scatter Chart - Legend & Ratio Debug").size(1000.0, 700.0),
        |cx| cx.new(ScatterDebugApp::new),
    );
}

struct ScatterDebugApp {
    // Data - multiple series
    x1: Vec<f64>,
    y1: Vec<f64>,
    x2: Vec<f64>,
    y2: Vec<f64>,
...
impl ScatterDebugApp {
    fn new(_cx: &mut Context<Self>) -> Self {
        // Generate sample data - three clusters
        let (x1, y1) = generate_cluster(30.0, 60.0, 15.0, 40);
        let (x2, y2) = generate_cluster(70.0, 40.0, 12.0, 35);
        let (x3, y3) = generate_cluster(50.0, 80.0, 10.0, 30);
                ...
                div()
                    ...
                    .child(
                        div()
                            ...
                            .child(div().flex().gap_1().children((0..10).map(|i| {
                                let entity = entity.clone();
                ...
                div()
                    ...
                    .child(
                        "Select 'Auto' to let the chart automatically choose legend position \
                         based on the graph ratio. Circular indicators are used for scatter plots.",
                    ),
            )
    }
    ...
    fn render_chart(&self) -> impl IntoElement {
        ...
        let mut chart = scatter(&self.x1, &self.y1)
            .label("Cluster A")
            .color(0x3b82f6)
            .point_radius(6.0)
            .opacity(0.7)
            .add_series(&self.x2, &self.y2, Some("Cluster B"), 0xff7f0e, 6.0, 0.7)
            .add_series(&self.x3, &self.y3, Some("Cluster C"), 0x2ca02c, 6.0, 0.7)
...
impl Render for ScatterDebugApp {
                ...
                div()
                    ...
                    .child(
                        div()
                            ...
                            .child("Scatter Chart - Legend & Graph Ratio Debug"),
                    )
...
/// Generate a cluster of points around a center with some spread
fn generate_cluster(cx: f64, cy: f64, spread: f64, count: usize) -> (Vec<f64>, Vec<f64>) {
    ...
    for i in 0..count {
        ...
        let angle = (i as f64 / count as f64) * 2.0 * PI + (i as f64 * 0.7);
        let radius = spread * (0.3 + 0.7 * ((i as f64 * 1.3).sin().abs()));

        x.push(cx + radius * angle.cos());
        y.push(cy + radius * angle.sin());
    }

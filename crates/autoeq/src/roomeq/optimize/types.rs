...
use super::super :: pipeline :: { PipelineControl , PipelineEvent , PipelineObserver , PipelineStepId , PipelineStepStatus } ;
...
pub(super) fn emit_pipeline_event(observer: &SharedPipelineObserver, event: PipelineEvent) -> Result<()> {
    ...
    for res in results {
        ...
        if let Some(generated) = post_generated_fir {
            if let Some(chain) = channel_chains.get_mut(&channel_name) {
                chain.plugins.push(super::super::output::create_convolution_plugin(
                    &generated.filename,
                ));
            }

...
mod tests {
    ...
    fn test_stereo_2_0_level_alignment() {
        ...
        let result = autoeq::roomeq::optimize_room(&config, 48000.0, None, None).expect("Optimization failed");
        ...
        assert!(l_gain.abs() < 0.1, "Left should not be gained (it is lowest)");
        assert!((r_gain - -5.0).abs() < 0.1, "Right should be attenuated by -5dB");
    }
    ...
    fn test_stereo_2_1_workflow() {
        ...
        let mut speakers = HashMap::new();
        speakers.insert("l".to_string(), SpeakerConfig::Single(MeasurementSource::InMemory(make_test_curve(80.0))));
        speakers.insert("r".to_string(), SpeakerConfig::Single(MeasurementSource::InMemory(make_test_curve(80.0))));
        speakers.insert("sub".to_string(), SpeakerConfig::Single(MeasurementSource::InMemory(make_test_curve(90.0))));

...
pub(super) fn enable_multi_measurement_paths(config: &mut RoomConfig, fem_dir: &Path, fem_subdir: &str) {
...
pub(super) fn enable_multisub_multi_seat_paths(config: &mut RoomConfig, fem_dir: &Path, fem_subdir: &str) {
    let data_dir = fem_dir.join(fem_subdir);

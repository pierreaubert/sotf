//! Convergence Test - Check if error decreases with mesh refinement
//!
//! Tests BEM accuracy vs Mie theory at different mesh resolutions
...
fn main() {
    ...
    println!("=== Convergence Test (β = 2i/k) ===\n");
    ...
    for subdivisions in [2, 3, 4] {
        ...
        let bem_front = front_p.iter().sum::<f64>() / front_p.len().max(1) as f64;
        let bem_back = back_p.iter().sum::<f64>() / back_p.len().max(1) as f64;

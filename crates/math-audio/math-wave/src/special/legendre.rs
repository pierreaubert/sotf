...
mod tests {
    ...
    fn test_legendre_at_one() {
        ...
        for (n, &pn) in p.iter().enumerate().take(10) {
            assert!((pn - 1.0).abs() < 1e-10, "P_{}(1) = {} != 1", n, pn);
        }
    }
    ...
    fn gauss_legendre_points(n: usize) -> (Vec<f64>, Vec<f64>) {
        ...
        let len = n.min(20);
        points[..len].copy_from_slice(&gl20_x[..len]);
        weights[..len].copy_from_slice(&gl20_w[..len]);

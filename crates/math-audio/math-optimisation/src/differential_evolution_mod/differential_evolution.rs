    ...
    F: Fn(&Array1<f64>) -> f64 + Sync,
{
    ...
    pub fn solve(&mut self) -> DEReport {
        ...
        let external_archive: Option<Arc<RwLock<super::super::external_archive::ExternalArchive>>> = if matches!(
            self.config.strategy,
            Strategy::LShadeBin | Strategy::LShadeExp
        ) {
            ...
            let (trials, trial_params): (Vec<_>, Vec<_>) = (0..npop)
                ...
                .map(|i| {
                    ...
                    let (mutant, cross) = match self.config.strategy {
                        ...
                        Strategy::LShadeBin => {
                            let pbest_size = super::super::mutant_current_to_pbest1::compute_pbest_size(

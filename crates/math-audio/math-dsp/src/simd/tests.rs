...
fn test_blend_simd_known_values() {
    ...
    for i in 0..dst.len() {
        ...
        assert!((dst[i] - expected).abs() < 1e-6, "index {i}: got {} expected {}", dst[i], expected);
    }
}
...
fn test_fast_inv_sqrt_accuracy() {
    for x in [1.0f32, 4.0, 9.0, 16.0, 100.0, 0.5, 2.0] {
        ...
        assert!(rel_err < 2e-3, "x={x} got={got} expected={expected} rel_err={rel_err}");
    }
}
...
fn test_find_max_abs_simd_unaligned() {
    for size in [1usize, 3, 5, 7, 9, 13, 17] {
        let samples: Vec<f32> = (0..size).map(|i| (i as f32) * (-1.0f32).powi(i as i32)).collect();
        let expected = samples.iter().map(|&s| s.abs()).fold(0.0f32, f32::max);
        assert!((find_max_abs_simd(&samples) - expected).abs() < 1e-6, "size {size}");
    }
}

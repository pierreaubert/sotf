...
fn test_cjk_optimal_falls_back_to_greedy() {
    ...
    println!("greedy lines: {:?}", greedy.lines.iter().map(|l| l.text.clone()).collect::<Vec<_>>());
    println!("optimal lines: {:?}", optimal.lines.iter().map(|l| l.text.clone()).collect::<Vec<_>>());
    ...
    assert_eq!(greedy.line_count, optimal.line_count, "optimal should handle CJK without fallback");
}

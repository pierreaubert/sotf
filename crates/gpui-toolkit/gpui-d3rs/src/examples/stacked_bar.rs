...
pub fn default_data() -> (Vec<String>, Vec<String>, Vec<Vec<f64>>) {
    let states: Vec<String> = ["California", "Texas", "Florida", "New York", "Illinois"]
        ...
        .map(|s| s.to_string())
        .collect();
    let ages: Vec<String> = ["<10", "10-19", "20-29", "30-39", "40-49", "50-59", "60-69", "70+"]
        .iter()
        .map(|s| s.to_string())
        .collect();
    let matrix = vec![
        vec![5038.0, 5170.0, 5765.0, 5430.0, 5044.0, 4835.0, 3738.0, 2920.0],
        vec![3983.0, 3862.0, 3872.0, 3678.0, 3360.0, 3092.0, 2388.0, 1708.0],
        vec![2211.0, 2331.0, 2641.0, 2574.0, 2524.0, 2685.0, 2462.0, 2285.0],
        vec![2334.0, 2470.0, 2903.0, 2700.0, 2523.0, 2706.0, 2128.0, 1709.0],
        vec![1625.0, 1710.0, 1826.0, 1699.0, 1591.0, 1688.0, 1259.0, 953.0],
    ];
    (states, ages, matrix)
}

...
pub fn compute(data: &[DiffChartRow]) -> DiffChartResult {
    ...
    // Build "above" area: fill where value0 > value1 (warmer than normal)
    // Forward pass: top edge = value0
    // Backward pass: bottom edge = min(value0, value1)
    //   → when v0 > v1: bottom = v1, shows the difference
    //   → when v1 >= v0: bottom = v0, area collapses to zero (hidden)
    let mut above_builder = PathBuilder::new();
    for (i, d) in data.iter().enumerate() {
        let x = x_scale.scale(i as f64);
        let y = y_scale.scale(d.value0);
        if i == 0 {
    ...
    for (i, d) in data.iter().enumerate().rev() {
        let x = x_scale.scale(i as f64);
        let y = y_scale.scale(d.value0.min(d.value1));
        above_builder = above_builder.line_to(x, y);
    }
    above_builder = above_builder.close_path();

    // Build "below" area: fill where value1 > value0 (cooler than normal)
    // Forward pass: top edge = value1
    // Backward pass: bottom edge = min(value0, value1)
    //   → when v1 > v0: bottom = v0, shows the difference
    //   → when v0 >= v1: bottom = v1, area collapses to zero (hidden)
    let mut below_builder = PathBuilder::new();
    for (i, d) in data.iter().enumerate() {
        let x = x_scale.scale(i as f64);
        let y = y_scale.scale(d.value1);
        if i == 0 {
    ...
    // Line paths for the two series
    let mut line0 = PathBuilder::new();
    let mut line1 = PathBuilder::new();
    for (i, d) in data.iter().enumerate() {
        let x = x_scale.scale(i as f64);
        let y0 = y_scale.scale(d.value0);
        let y1 = y_scale.scale(d.value1);
        if i == 0 {
            line0 = line0.move_to(x, y0);
            line1 = line1.move_to(x, y1);
        } else {
            line0 = line0.line_to(x, y0);
            line1 = line1.line_to(x, y1);
        }
    }

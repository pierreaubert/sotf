...
pub fn render(_app: &ShowcaseApp, _cx: &mut Context<ShowcaseApp>) -> Div {
    ...
    d3_paths.push(
        D3PathBuilder::new()
            .move_to(0.0, 0.0).line_to(width, 0.0).line_to(width, height).line_to(0.0, height)
            .close_path().build(),
    );
    ...
    for star in &result.stars {
        let path = D3PathBuilder::new()
            ...
            .build();
        d3_paths.push(path);
        all_colors.push(Hsla::from(Rgba { r: star.r as f32 / 255.0, g: star.g as f32 / 255.0, b: star.b as f32 / 255.0, a: 0.8 }));
    }
    ...
    div()
        ...
        .child(div().text_xs().text_color(rgb(0x666666)).mb_2()
            .child(format!("Source: observablehq.com/@d3/hertzsprung-russell-diagram — {} stars", data.len())))
        .child(
            div().w(px(width as f32)).h(px(height as f32)).bg(rgb(0x111111)).border_1().border_color(rgb(0x333333))
                .relative()
                .child(canvas(
                    move |bounds, _, _| {
                ...
                .children((-7..=19).step_by(2).map(|mag| {
                    ...
                    div().absolute().left(px(2.0)).top(px((y - 5.0) as f32))
                        .text_size(px(8.0)).text_color(rgb(0x888888))
                        .child(format!("{mag}"))
                }))
                // X-axis: B-V color index
                .children([0.0, 0.5, 1.0, 1.5, 2.0].iter().map(|&bv| {
                    let x = 40.0 + (bv + 0.39) / 2.58 * (width - 80.0);
                    div().absolute().left(px((x - 8.0) as f32)).top(px((height - 15.0) as f32))
                        .text_size(px(8.0)).text_color(rgb(0x888888))
                        .child(format!("{bv:.1}"))
                }))
                // X-axis: temperature (top axis)
                .children([30000, 10000, 7000, 5000, 3500].iter().enumerate().map(|(i, &temp)| {
                    let bv = [0.0_f64, 0.5, 0.8, 1.2, 1.8][i];
                    let x = 40.0 + (bv + 0.39) / 2.58 * (width - 80.0);
                    div().absolute().left(px((x - 15.0) as f32)).top(px(5.0))
                        .text_size(px(8.0)).text_color(rgb(0x888888))
                        .child(format!("{temp}K"))
                }))
                // Axis labels
                .child(div().absolute().left(px(2.0)).top(px((height / 2.0 - 30.0) as f32))
                    .text_size(px(9.0)).text_color(rgb(0xaaaaaa)).child("← Brighter"))
                .child(div().absolute().right(px(5.0)).top(px(20.0))
                    .text_size(px(9.0)).text_color(rgb(0xaaaaaa)).child("Temperature →"))
                .child(div().absolute().left(px((width / 2.0 - 30.0) as f32)).bottom(px(2.0))
                    .text_size(px(9.0)).text_color(rgb(0xaaaaaa)).child("B-V Color Index")),
        )
}

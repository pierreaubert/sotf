...
pub fn render(_app: &ShowcaseApp, _cx: &mut Context<ShowcaseApp>) -> Div {
    ...
    div()
        ...
        .child(div().text_lg().font_weight(FontWeight::BOLD).mb_2().child("Electric Usage 2019"))
        .child(div().text_xs().text_color(rgb(0x666666)).mb_2()
            .child(format!("Source: observablehq.com/@mbostock/electric-usage-2019 — {} hourly readings, {} days",
                data.len(), result.unique_dates)))
        .child(
            div().w(px(width as f32)).h(px(height as f32)).bg(rgb(0xffffff)).border_1().border_color(rgb(0xe0e0e0))
                ...
                .children((0..24).step_by(3).map(|h| {
                    ...
                    div().absolute().left(px((x + result.cell_width / 2.0 - 5.0) as f32)).top(px(10.0))
                        .text_size(px(8.0)).text_color(rgb(0x666666))
                        .child(format!("{h}:00"))
                }))
                ...
                .children({
                    ...
                    (0..12).map(move |m| {
                        ...
                        div().absolute().left(px(5.0)).top(px(y as f32))
                            .text_size(px(8.0)).text_color(rgb(0x666666))
                ...
                // Color legend
                .child(
                    div().absolute().right(px(5.0)).top(px(10.0))
                        .flex().items_center().gap_1()
                        .child(div().text_size(px(7.0)).text_color(rgb(0x999999)).child("Low"))
                        .child(div().flex().h(px(8.0)).w(px(60.0)).rounded_sm().overflow_hidden()
                            .children((0..10).map(|i| {
                                let t = i as f64 / 9.0;
                                div().flex_1().h_full().bg(scheme.get(t).to_rgba())
                            })))
                        .child(div().text_size(px(7.0)).text_color(rgb(0x999999)).child("High"))
                ),
        )
}

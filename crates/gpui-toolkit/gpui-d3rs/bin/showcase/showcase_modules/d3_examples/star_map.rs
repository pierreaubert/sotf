...
pub fn render(_app: &ShowcaseApp, _cx: &mut Context<ShowcaseApp>) -> Div {
    ...
    for star in &result.stars {
        ...
        all_colors.push(hsla(0.15, 0.2 * (1.0 - brightness) as f32, (0.5 + 0.5 * brightness) as f32, 1.0));
    }

    div()
        .flex().flex_col().size_full().p_4()
        .child(div().text_lg().font_weight(FontWeight::BOLD).mb_2().child("Star Map"))
        .child(div().text_xs().text_color(rgb(0x666666)).mb_2()
            .child(format!("Source: observablehq.com/@d3/star-map — {} stars visible", result.stars.len())))
        .child(
            div().w(px(width as f32)).h(px(height as f32)).bg(rgb(0x0a0a1a)).border_1().border_color(rgb(0x333333))
                .child(canvas(
                    move |bounds, _, _| {

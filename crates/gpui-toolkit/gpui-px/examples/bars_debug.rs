//! Interactive Bar Chart Debug - Demonstrates grouped bars, legend position and graph ratio controls.
//!
...
fn main() {
    MiniApp::run(
        MiniAppConfig::new("Bar Chart - Legend & Ratio Debug").size(1000.0, 700.0),
        |cx| cx.new(BarsDebugApp::new),
    );
}

struct BarsDebugApp {
    // Data - categories and multiple series
...
impl BarsDebugApp {
    fn new(_cx: &mut Context<Self>) -> Self {
        // Generate sample data - quarterly sales by product
        let categories = vec![
            "Q1".to_string(),
            "Q2".to_string(),
            "Q3".to_string(),
        ...
        let values1 = vec![45.0, 55.0, 70.0, 85.0]; // Product A
        let values2 = vec![60.0, 48.0, 65.0, 78.0]; // Product B
        let values3 = vec![35.0, 52.0, 45.0, 68.0]; // Product C
                ...
                div()
                    ...
                    .child(
                        "Select 'Auto' to let the chart automatically choose legend position \
                         based on the graph ratio. Grouped bars are rendered side-by-side within each category.",
                    ),
            )
    }
    ...
    fn render_chart(&self) -> impl IntoElement {
        ...
        let mut chart = bar(&self.categories, &self.values1)
            .label("Product A")
            .color(0x3b82f6)
            .add_series(&self.values2, Some("Product B"), 0xff7f0e, 0.8)
            .add_series(&self.values3, Some("Product C"), 0x2ca02c, 0.8)
            .title("Quarterly Sales by Product (Grouped Bars)")
                ...
                div()
                    ...
                    .child(
                        div()
                            ...
                            .child("Bar Chart - Legend & Graph Ratio Debug"),
                    )

...
mod tests {
    ...
    #[test]
    fn test_heatmap_responsive_size_defaults_and_fixed_opt_in() {
        let z = vec![1.0; 9];

        crate::assert_default_chart_size(heatmap(&z, 3, 3).chart_size);
        crate::assert_fixed_chart_size(
            heatmap(&z, 3, 3).size(360.0, 240.0).chart_size,
            360.0,
            240.0,
        );

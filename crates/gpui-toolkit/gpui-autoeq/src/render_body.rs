{
        ...
        if show_eq_design {
            ...
            let opt_mode_options: Vec<SelectOption> = OPT_MODE_OPTIONS
                .iter()
                .filter(|(val, _)| {
                    if let Some(allowed) = &allowed_opt_modes {
        ...
        // ========================================
        // Optimizer Settings & Target/Loss Sections
        // ========================================
        if show_optimization_tuning {
            ...
            // Target & Loss Section
            // ========================================
            let mut target_loss_content = VStack::new().spacing(StackSpacing::Sm);
            ...
            target_loss_content = target_loss_content.child(
                VStack::new()
                    .spacing(StackSpacing::None)
                    .child(
                        Text::new("Target & Loss")
                            .size(TextSize::Sm)
                            .weight(TextWeight::Semibold)
                            .color(theme.header_color),
                    )
                    .child(
                        Text::new("Configure how the optimizer evaluates the correction quality")
                            .size(TextSize::Xs)
                            .color(theme.description_color),
                    ),
            );
        ...
        if optimization_type == OptimizationType::Speaker && !hide_room_sections {
            ...
            // Header
            {
                ...
                advanced_a_content = advanced_a_content.child(header);
            }

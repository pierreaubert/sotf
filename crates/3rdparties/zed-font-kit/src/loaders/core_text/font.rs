...
use crate :: properties :: { Properties , Style } ;
use crate::utils;
use super::cgpoint_ext::CGPointExt;
use super::consts::OTTO_TAG;
use super::consts::TTC_TAG;
use super::consts::format_to_cg_color_space_and_image_format;
...
use super::unpack::unpack_data_fork_font;
use super::unpack::unpack_otc_font;

/// A loader that uses Apple's Core Text API to load and rasterize fonts.
#[derive(Clone)]
pub struct Font {
    pub(super) core_text_font: CTFont,
    pub(super) font_data: FontData,
}
...
pub(super) fn font_is_collection(header: &[u8]) -> bool {
    header.len() >= 4 && header[0..4] == TTC_TAG
}

pub(super) fn read_number_of_fonts_from_otc_header(header: &[u8]) -> Result<u32, FontLoadingError> {

...
// SotF vendor patch (2026-05-08): the upstream gpui_macos crate declared
// the private CGS symbols `CGSMainConnectionID` and
// `CGSSetWindowBackgroundBlurRadius` here and called them on macOS < 12.0
// for window background blur. Apple's Mac App Store static analyzer
// rejects ANY reference to these symbols (private SPI), even from a
// branch that's unreachable on our supported targets (LSMinimumSystemVersion
// = 13.0). The corresponding call in `set_background_appearance` is

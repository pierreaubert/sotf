...
fn main() {
    ...
    while pos < num_frames {
        ...
        plugin_mono.process(&input[pos..end], &mut output[pos*2..end*2], &ctx).unwrap();
        pos = end;
    }
    ...
    let mut plugin = MonoToStereoPlugin::from_params(1, MonoToStereoPluginParams { stereo_width: 1.0, comp_eq_depth_db: 0.0 });
    plugin.initialize(sample_rate).unwrap();
    ...
    while pos < num_frames {
        let end = (pos + block_size).min(num_frames);
        let ctx = ProcessContext { sample_rate, num_frames: end - pos };
        plugin.process(&input[pos..end], &mut output_stereo[pos*2..end*2], &ctx).unwrap();
        pos = end;
    }

...
The upmixer uses an optional ML model to detect vocal content in real time,
improving center-channel steering over the default heuristic detector.
The model is a tiny 3-layer MLP (~1857 parameters) that runs on a dedicated
thread via ONNX Runtime, completely off the audio thread.
...
FFT bins ──► MfccExtractor
             │
             ▼
         [20 MFCCs + 20 deltas] ──► rtrb ring buffer ──► ONNX session
                                                          │
         V_prob (atomic f32) ◄────────────────────────────┘
```
...
| File              | Role                                                    |
|-------------------|---------------------------------------------------------|
| `ml_features.rs`  | MFCC extractor (40-element feature vector, zero-alloc)  |
...
- **Input**: `"input"` — shape `[1, 40]` float32 (20 MFCCs + 20 delta MFCCs)
...
3. **Power spectrum**: `|X[k]|^2` (mono average for stereo: `0.5 * (|L|^2 + |R|^2)`)
...
This will:
1. Extract MFCC features from all 7 demo WAV files in `crates/app-gpui/assets/demo-audio/`
2. Generate pseudo-labels using [Silero VAD](https://github.com/snakers4/silero-vad)
3. Train a 3-layer MLP with early stopping
4. Export to ONNX with sigmoid wrapper
5. Validate shapes, range, and latency with ONNX Runtime

Output: `crates/plugins/models/vocal_detector.onnx`
...
Silero VAD provides frame-level speech/non-speech labels (~13,500 frames total).
The training uses an 80/20 stratified split with class-weighted BCE loss.
...
- Expected validation accuracy: ~96% (demo), higher with external data
...
let params = UpmixerPluginParams {
    enable_ml_detection: true,
    ml_model_path: "crates/plugins/models/vocal_detector.onnx".into(),
    ..Default::default()
};
```
...
import numpy as np
s = ort.InferenceSession('crates/plugins/models/vocal_detector.onnx')
print('Input:', s.get_inputs()[0].name, s.get_inputs()[0].shape)
print('Output:', s.get_outputs()[0].name, s.get_outputs()[0].shape)
r = s.run(None, {'input': np.zeros((1,40), dtype=np.float32)})[0]
...
cargo test -p plugins --no-default-features --lib -- ml_features

# ML inference tests (requires dummy model in test_data/)
cargo test -p plugins --no-default-features --lib -- ml_inference

# Full upmixer test suite
cargo test -p plugins --no-default-features --lib -- upmixer
```

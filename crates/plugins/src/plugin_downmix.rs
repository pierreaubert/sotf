...
fn default_center_gain_db() -> f32 { CENTER_GAIN_DB_DEFAULT }
fn default_surround_gain_db() -> f32 { SURROUND_GAIN_DB_DEFAULT }
fn default_height_gain_db() -> f32 { HEIGHT_GAIN_DB_DEFAULT }
fn default_lfe_gain_db() -> f32 { LFE_GAIN_DB_DEFAULT }
fn default_phase_coherence() -> bool { PHASE_COHERENCE_DEFAULT }
fn default_phase_blend_low_hz() -> f32 { PHASE_BLEND_LOW_HZ_DEFAULT }
fn default_phase_blend_high_hz() -> f32 { PHASE_BLEND_HIGH_HZ_DEFAULT }
...
impl DownmixPlugin {
    pub fn new(input_channels: usize) -> Self {
        ...
        let mut plugin = Self {
            ...
            out_freq: [vec![Complex::new(0.0, 0.0); freq_len], vec![Complex::new(0.0, 0.0); freq_len]],
    ...
    fn compute_coefficients(&mut self, reset: bool) {
        ...
        if let Some(config) = self.speaker_config {
            for s in config.speakers {
                if s.is_lfe {
                    ...
                    new_coeffs.push(DownmixCoeffs { left_gain: l_lin * 0.5, right_gain: l_lin * 0.5 });
                } else {
                    let cat_gain = if s.elevation.abs() > 10.0 { h_lin }
                        else if s.azimuth.abs() < 15.0 { c_lin }
                        else if s.azimuth.abs() > 60.0 { s_lin }
                        else { 1.0 };
                    let ang = (s.azimuth.to_radians() + std::f32::consts::FRAC_PI_2) * 0.5;
                    new_coeffs.push(DownmixCoeffs { left_gain: (cat_gain * ang.sin()).max(0.0), right_gain: (cat_gain * ang.cos()).max(0.0) });
                }
            }
        } else {
            for ch in 0..self.input_ch {
                if self.input_ch == 1 { new_coeffs.push(DownmixCoeffs { left_gain: 1.0, right_gain: 1.0 }); }
                else {
        ...
        let max_sum = new_coeffs.iter().map(|c| c.left_gain).sum::<f32>().max(new_coeffs.iter().map(|c| c.right_gain).sum::<f32>());
        if max_sum > 1.0 {
            for c in &mut new_coeffs { c.left_gain /= max_sum; c.right_gain /= max_sum; }
        }
        ...
        if self.coeff_smoothers.is_empty() {
            ...
            for (i, c) in self.target_coeffs.iter().enumerate() {
                if reset { self.coeff_smoothers[i*2].reset(c.left_gain); self.coeff_smoothers[i*2+1].reset(c.right_gain); }
                else { self.coeff_smoothers[i*2].set_target(c.left_gain); self.coeff_smoothers[i*2+1].set_target(c.right_gain); }
            }
        }
    }
    ...
    fn process_simple(&mut self, input: &[f32], output: &mut [f32], num_frames: usize) {
        for frame in 0..num_frames {
            ...
            output[frame * 2] = l; output[frame * 2 + 1] = r;
        }
    }

    fn process_fft_block(&mut self) {
        let n = FFT_SIZE; let inv_n = 1.0 / n as f32;
        for ch in 0..self.input_ch {
            for i in 0..n { self.fft_input_buf[i] = self.input_ring[ch][(self.input_ring_pos + i) % n] * self.window[i]; }
        ...
        for bin in 0..(n/2+1) {
            ...
            let blend = if freq <= self.phase_blend_low_hz { 0.0 } else if freq >= self.phase_blend_high_hz { 1.0 }
                else { let t = (freq - self.phase_blend_low_hz) / (self.phase_blend_high_hz - self.phase_blend_low_hz); t*t*(3.0-2.0*t) };
            let mut sl = Complex::new(0.0, 0.0); let mut sr = Complex::new(0.0, 0.0);
            let mut ml = -1.0f32; let mut mr = -1.0f32; let mut pl = 0.0f32; let mut pr = 0.0f32;
            for ch in 0..self.input_ch {
                let gl = self.coeff_smoothers[ch*2].current(); let gr = self.coeff_smoothers[ch*2+1].current();
                let wl = self.channel_freq[ch][bin] * gl; let wr = self.channel_freq[ch][bin] * gr;
                sl += wl; sr += wr;
                if blend > 0.0 {
                    let nml = wl.norm(); if nml > ml { ml = nml; pl = wl.arg(); }
                    let nmr = wr.norm(); if nmr > mr { mr = nmr; pr = wr.arg(); }
                }
            }
            if blend < 0.001 { self.out_freq[0][bin] = sl; self.out_freq[1][bin] = sr; }
            else {
                let mut al = Complex::new(0.0, 0.0); let mut ar = Complex::new(0.0, 0.0);
                for ch in 0..self.input_ch {
                    al += Complex::from_polar((self.channel_freq[ch][bin] * self.coeff_smoothers[ch*2].current()).norm(), pl);
                    ar += Complex::from_polar((self.channel_freq[ch][bin] * self.coeff_smoothers[ch*2+1].current()).norm(), pr);
                }
                self.out_freq[0][bin] = sl * (1.0 - blend) + al * blend; self.out_freq[1][bin] = sr * (1.0 - blend) + ar * blend;
            }
        }
        self.out_freq[0][0].im = 0.0; self.out_freq[0][n/2].im = 0.0;
        self.out_freq[1][0].im = 0.0; self.out_freq[1][n/2].im = 0.0;
        for lr in 0..2 {
            ...
            let write_pos = self.output_write_pos; let accum_len = self.output_accum[0].len();
            for i in 0..n { self.output_accum[lr][(write_pos + i) % accum_len] += self.ifft_output_buf[i] * inv_n; }
        }
...
impl Plugin for DownmixPlugin {
    fn info(&self) -> PluginInfo { PluginInfo::new("Downmix", "1.1.0", "SotF") }
    fn input_channels(&self) -> usize { self.input_ch }
    fn output_channels(&self) -> usize { 2 }
    fn parameters(&self) -> Vec<Parameter> {
        vec![
            Parameter::new_float("center_gain_db", "Center Gain", CENTER_GAIN_DB_DEFAULT, CENTER_GAIN_DB_MIN, CENTER_GAIN_DB_MAX),
            Parameter::new_float("surround_gain_db", "Surround Gain", SURROUND_GAIN_DB_DEFAULT, SURROUND_GAIN_DB_MIN, SURROUND_GAIN_DB_MAX),
            Parameter::new_float("height_gain_db", "Height Gain", HEIGHT_GAIN_DB_DEFAULT, HEIGHT_GAIN_DB_MIN, HEIGHT_GAIN_DB_MAX),
            Parameter::new_float("lfe_gain_db", "LFE Gain", LFE_GAIN_DB_DEFAULT, LFE_GAIN_DB_MIN, LFE_GAIN_DB_MAX),
            Parameter::new_bool("phase_coherence", "Phase Coherence", PHASE_COHERENCE_DEFAULT),
        ]
    }
    fn set_parameter(&mut self, id: ParameterId, value: ParameterValue) -> PluginResult<()> {
        if id == self.param_center_gain_db { self.center_gain_db = value.as_float().ok_or("val")?; }
        else if id == self.param_surround_gain_db { self.surround_gain_db = value.as_float().ok_or("val")?; }
        else if id == self.param_height_gain_db { self.height_gain_db = value.as_float().ok_or("val")?; }
        else if id == self.param_lfe_gain_db { self.lfe_gain_db = value.as_float().ok_or("val")?; }
        else if id == self.param_phase_coherence { self.phase_coherence = value.as_bool().ok_or("val")?; }
        else { return Err("unknown".into()); }
        self.compute_coefficients(false); Ok(())
    }
    ...
    fn initialize(&mut self, sample_rate: u32) -> PluginResult<()> {
        ...
        self.lfe_lpf = self.lfe_channels.iter().map(|_| [Biquad::new(BiquadFilterType::Lowpass, 120.0, sample_rate as f64, 0.0, 0.0), Biquad::new(BiquadFilterType::Lowpass, 120.0, sample_rate as f64, 0.0, 0.0)]).collect();
    ...
    fn process(&mut self, input: &[f32], output: &mut [f32], context: &ProcessContext) -> Result<usize, String> {
        ...
        if !self.phase_coherence { self.process_simple(input, output, num_frames); return Ok(num_frames); }
        for frame in 0..num_frames {
            for ch in 0..self.input_ch {
                let mut s = input[frame * self.input_ch + ch];
                if let Some(li) = self.lfe_channels.iter().position(|&c| c == ch) {
                    if li < self.lfe_lpf.len() {
                        let mut v = s as f64; v = self.lfe_lpf[li][0].process(v); v = self.lfe_lpf[li][1].process(v); s = v as f32;
                    }
                }
            ...
            self.input_fill += 1;
            if self.input_fill >= HOP_SIZE { self.input_fill = 0; self.process_fft_block(); }
            let rp = self.output_read_pos;
            output[frame * 2] = self.output_accum[0][rp]; output[frame * 2 + 1] = self.output_accum[1][rp];
            self.output_accum[0][rp] = 0.0; self.output_accum[1][rp] = 0.0;
            self.output_read_pos = (rp + 1) % self.output_accum[0].len();
            for s in &mut self.coeff_smoothers { s.next(); }
        }
        Ok(num_frames)
    }
    fn latency_samples(&self) -> usize { if self.phase_coherence { FFT_SIZE } else { 0 } }
}
...
mod tests {
    ...
    fn test_downmix_basic() {
        let mut p = DownmixPlugin::new(2); p.initialize(44100).unwrap(); p.phase_coherence = false;
        let mut i = vec![0.0; 2048]; let mut o = vec![0.0; 2048];
        for k in 0..1024 { i[k*2] = (k as f32 * 0.01).sin(); i[k*2+1] = (k as f32 * 0.02).sin(); }
        p.process(&i, &mut o, &ProcessContext { sample_rate: 44100, num_frames: 1024 }).unwrap();
        assert!(o.iter().any(|&s| s.abs() > 1e-5));
    }
    #[test]
    fn test_downmix_51() {
        let mut p = DownmixPlugin::new(6); p.phase_coherence = false; p.initialize(44100).unwrap();
        let mut i = vec![0.0; 600]; let mut o = vec![0.0; 200];
        for k in 0..100 { i[k*6 + 2] = 1.0; }
        p.process(&i, &mut o, &ProcessContext { sample_rate: 44100, num_frames: 100 }).unwrap();
        assert!(o[0].abs() > 0.01);
    }
}

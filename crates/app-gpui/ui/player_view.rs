...
pub struct PlayerView {
    ...
    pub(super) geometry_save_task: Option<Task<()>>,
    ...
    pub(super) geometry_save_pending: bool,
    ...
    pub(super) geometry_save_sequence: u64,
    /// Latest geometry the user moved to; the in-flight save task drains
    /// this when its timer fires.
    pub(super) pending_geometry_save:
        std::sync::Arc<parking_lot::Mutex<Option<PendingGeometrySave>>>,
    ...
    /// graph changes.
    pub(super) compressor_engine_idx_cache: Option<Option<usize>>,
    ...
    pub(super) midi_input: Option<crate::app::midi_input::MidiInputService>,
...
impl PlayerView {
    ...
    pub(super) fn dispatch_midi_messages(
        state: &mut AppState,
    ...
    #[cfg(target_os = "ios")]
    pub(super) fn drain_ios_remote_commands(state: &mut AppState) {
        for _ in 0..32 {
    ...
    #[cfg(target_os = "ios")]
    pub(super) fn handle_ios_imported_files(state: &mut AppState) {
    ...
    #[cfg(target_os = "ios")]
    pub(super) fn handle_ios_scanned_qr(state: &mut AppState) {
        // SAFETY: implemented by app-ios in the final binary. It returns an
    ...
    pub(super) fn open_config(&mut self, _: &OpenConfig, _: &mut Window, cx: &mut Context<Self>) {
    ...
    pub(super) fn set_design_system(
        &mut self,
        name: &str,
    ...
    pub(super) fn set_design_neutral(
        &mut self,
        _: &SetDesignNeutral,
        window: &mut Window,
        cx: &mut Context<Self>,
    ...
    pub(super) fn set_design_apple_hig(
        &mut self,
        _: &SetDesignAppleHig,
        window: &mut Window,
        cx: &mut Context<Self>,
    ...
    pub(super) fn set_design_material3(
        &mut self,
    ...
    pub(super) fn set_design_fluent(
        &mut self,
        _: &SetDesignFluent,
        window: &mut Window,
        cx: &mut Context<Self>,
    ...
    pub(super) fn toggle_library_view(
        &mut self,
        _: &ToggleLibraryView,
        _: &mut Window,
    ...
    pub(super) fn toggle_help(&mut self, _: &ToggleHelp, _: &mut Window, cx: &mut Context<Self>) {
        self.state.update(cx, |state, _cx| {
    ...
    pub(super) fn toggle_help_support(
        &mut self,
        _: &ToggleHelpSupport,
        _: &mut Window,
        cx: &mut Context<Self>,
    ...
    pub(super) fn toggle_screen_guide(
        &mut self,
        _: &ToggleScreenGuide,
        _: &mut Window,
        cx: &mut Context<Self>,
    ...
    pub(super) fn about(&mut self, _: &About, _: &mut Window, cx: &mut Context<Self>) {
        self.state.update(cx, |state, _cx| {
            use crate::app::InputMode;
    ...
    pub(super) fn clear_queue(&mut self, _: &ClearQueue, _: &mut Window, cx: &mut Context<Self>) {
        self.state.update(cx, |state, _cx| {
    ...
    pub(super) fn play_now(&mut self, _: &PlayNow, _: &mut Window, cx: &mut Context<Self>) {
    ...
    pub(super) fn scan_library(&mut self, _: &ScanLibrary, _: &mut Window, cx: &mut Context<Self>) {
        self.state.update(cx, |state, _cx| {
    ...
    pub(super) fn play_track_at_with_mode(
        state: &mut AppState,
        source: sotf_audio::decoder::AudioSource,
        position: Option<f64>,
    ...
    pub(super) fn play_track_at_inner(
        state: &mut AppState,
        source: sotf_audio::decoder::AudioSource,
        position: Option<f64>,

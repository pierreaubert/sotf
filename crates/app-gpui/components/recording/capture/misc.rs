...
impl PlayerView {
    ...
    pub(super) fn render_signal_type_dropdown(&self, cx: &mut Context<Self>) -> impl IntoElement {
        let state = self.state.read(cx);
    ...
    pub(super) fn render_channel_config_section(&self, cx: &mut Context<Self>) -> impl IntoElement {
        let state = self.state.read(cx);
        let theme = state.app.ui_state.theme.clone();
        let translations = state.app.ui_state.translations.clone();
        let recording_state = &state.app.measurement_state.recording_state;
    ...
    pub(super) fn render_channel_status_section(&self, cx: &mut Context<Self>) -> impl IntoElement {
        let d = Ds::from_cx(cx);
        let state = self.state.read(cx);
        let theme = state.app.ui_state.theme.clone();
        let translations = state.app.ui_state.translations.clone();
    ...
    pub(super) fn render_channel_list(&self, cx: &mut Context<Self>) -> impl IntoElement {
    ...
    pub(super) fn render_capture_redo_actions(&self, cx: &mut Context<Self>) -> impl IntoElement {
        let state = self.state.read(cx);
        let theme = state.app.ui_state.theme.clone();
        let translations = state.app.ui_state.translations.clone();
        let recording_state = &state.app.measurement_state.recording_state;
    ...
    pub(super) fn ensure_named_recording_directory(
        &mut self,
        cx: &mut Context<Self>,
    ...
    pub(super) fn perform_migration(&mut self, cx: &mut Context<Self>) {
    ...
    /// from the Save step.
    pub(super) fn cancel_position_modal(&mut self, cx: &mut Context<Self>) {
        self.state.update(cx, |state, _| {
            let rec_state = &mut state.app.measurement_state.recording_state;
            rec_state.move_position_modal_open = false;

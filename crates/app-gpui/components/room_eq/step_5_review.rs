...
impl PlayerView {
    ...
    fn render_selected_channel_result(&self, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let has_fir = room_eq
            ...
            .as_ref()
            .and_then(|out| {
                super::room_eq_channel_chain_by_name(&out.channels, &result.channel_name)
            })
            .map(|chain| {
        ...
        if let Some(chain) = room_eq.dsp_output.as_ref().and_then(|out| {
            ...
            if let Some(points) = super::room_eq_initial_response_points(Some(chain), None) {
                display_result.original_response = Some(points);
            }
            if let Some(points) = super::room_eq_display_response_points(Some(chain), None) {

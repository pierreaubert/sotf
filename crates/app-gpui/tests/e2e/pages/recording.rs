...
impl<'a, 'b> RecordingPage<'a, 'b> {
    ...
    pub fn verify_recording_results(&mut self) {
        self.driver.read_app(|app| {
            for recording in &app.measurement_state.recording_state.channel_recordings {
                ...
                assert!(
                    recording.result.is_some(),
                    "Channel {} has no result",
                    recording.channel_name
                );
                ...
                .position(|d| d.name == device_name)
            {
                ...
                if let Some(device) = app.audio_device_state.output_devices.get(idx).cloned() {
                    ...
                    app.measurement_state
                        .recording_state
                        .playback_config
                        .device_id = device.device_id.unwrap_or(device.name);
                    // Set max channels
    ...
    pub fn set_speaker_config(&mut self, config: SpeakerConfiguration) {
        self.driver.update_app(move |app, _| {
            ...
            app.measurement_state
                .recording_state
                .playback_config
                .num_channels = config.channel_count();
    ...
    pub fn configure_signal(
        &mut self,
        signal_type: RecordingSignalType,
        duration: f32,
        level_db: f32,
    ) {
        self.driver.update_app(move |app, _| {
    ...
    pub fn get_signal_type(&mut self) -> RecordingSignalType {
        self.driver
            .read_app(|app| app.measurement_state.recording_state.signal_type)
    }
    ...
    pub fn get_signal_duration(&mut self) -> f32 {
        self.driver
            .read_app(|app| app.measurement_state.recording_state.signal_duration_secs)
    }
    ...
    pub fn get_signal_level(&mut self) -> f32 {
        self.driver
            .read_app(|app| app.measurement_state.recording_state.signal_level_db)
    }
    ...
    pub fn set_recording_directory(&mut self, path: &str) {
        self.driver.update_app(move |app, _| {
            app.measurement_state
                .recording_state
                .recording_base_directory = Some(path.to_string());
        });
    }

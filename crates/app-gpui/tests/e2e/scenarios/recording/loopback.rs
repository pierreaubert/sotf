...
impl TestScenario for LoopbackScenario {
    ...
    fn execute(
        ...
        if loopback_name.is_none() {
            eprintln!(
                "WARNING: No loopback device (BlackHole, Loopback, etc.) found. Skipping recording test."
            );
            return Ok(());
        }
        ...
        for config in SpeakerConfiguration::all() {
            ...
            if required_channels > max_channels {
                eprintln!(
                    "Skipping config {:?} (needs {} channels, device has {})",
                    config, required_channels, max_channels
                );
                continue;
            }
            ...
            loop {
                if start_time.elapsed() > timeout {
                    return Err(format!(
                        "Timeout waiting for recording completion of {:?}",
                        config
                    )
                    .into());
                }

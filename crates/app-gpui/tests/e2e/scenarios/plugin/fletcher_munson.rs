...
impl TestScenario for FletcherMunsonScenario {
    ...
    fn execute(&self, cx: &mut VisualTestContext, window: WindowHandle<PlayerView>) -> Result<(), Box<dyn Error>> {
        ...
        driver.update_app(|app, _cx| {
             ...
             let plugin_idx = app.plugin_state.plugin_chain.plugins()
                 .iter()
        ...
        driver.read_app(|app| {
            ...
            let plugin = app.plugin_state.plugin_chain.get_plugin(plugin_idx)
                .ok_or_else(|| "Plugin not found")?;
            
            if let PluginSettings::FletcherMunson { reference_level_db, .. } = plugin.settings {
                println!("Current Reference Level: {}", reference_level_db);
                if (reference_level_db - -20.0).abs() > 0.001 {
                     panic!("Reference level mismatch: expected -20.0, got {}", reference_level_db);
                }
            } else {
                 panic!("Expected FletcherMunson plugin, found {:?}", plugin.plugin_type());
            }
            Ok::<(), Box<dyn Error>>(()) 
        });
        ...
        driver.read_app(|app| {
            ...
            if let PluginSettings::FletcherMunson { band1_freq, .. } = plugin.settings {
                ...
                if (band1_freq - 80.0).abs() > 0.001 {
                     panic!("Band 1 frequency mismatch: expected 80.0, got {}", band1_freq);
                }
            } else {

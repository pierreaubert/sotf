...
use super::types::PendingByteRange;
    ...
    fn parse_master_playlist_empty_returns_none() {
        let playlist = "#EXTM3U\n";
        assert!(parse::parse_master_playlist(&base_url("root/master.m3u8"), playlist)
            .unwrap()
            .is_none());
    }
    ...
    fn parse_media_playlist_empty_segments_rejected() {
        let playlist = "#EXTM3U\n#EXT-X-TARGETDURATION:6\n";
        let err = parse::parse_media_playlist(&base_url("hls/live/index.m3u8"), playlist)
            .unwrap_err();
        assert_eq!(err.kind(), io::ErrorKind::InvalidData);
    }
    ...
    fn segment_key_distinguishes_byte_ranges() {
        ...
        let seg1 = HlsSegment::new(url.clone(), Some(HlsByteRange { offset: 0, length: 100 }));
        let seg2 = HlsSegment::new(url.clone(), Some(HlsByteRange { offset: 100, length: 100 }));

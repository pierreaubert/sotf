...
HTTP streaming input and live PCM output for the audio engine.
...
- `PcmStreamServer` -- Live processed PCM output over HTTP (`/stream.wav`, `/stream.raw`, `/status`)

## Module Layout

- `http_source.rs` -- HTTP MediaSource implementation
- `icy.rs` -- ICY metadata parser
- `server.rs` -- HTTP live PCM server implementation

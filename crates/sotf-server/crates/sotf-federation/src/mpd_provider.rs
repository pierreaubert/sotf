...
use crate::provider::{LibraryEvent, LibraryProvider, ProviderAlbum, ProviderCapabilities, ProviderError, ProviderFuture, ProviderTrack, SourceId, SourceType};
...
impl MpdProvider {
    ...
    async fn connect(&self) -> Result<BufStream<TcpStream>, ProviderError> {
        ...
        if !greeting.starts_with("OK MPD") {
            return Err(ProviderError::Network(format!("unexpected MPD greeting: {}", greeting.trim())));
        }

        // Send password if configured
        if let Some(ref password) = self.config.password {
            self.send_command(&mut reader, &format!("password \"{}\"", escape_mpd_string(password))).await?;
    ...
    async fn send_command(&self, reader: &mut BufStream<TcpStream>, cmd: &str) -> Result<(), ProviderError> {
        let mut full_cmd = cmd.to_string();
        full_cmd.push('\n');
        reader.write_all(full_cmd.as_bytes()).await
    ...
    async fn read_response(&self, reader: &mut BufStream<TcpStream>) -> Result<String, ProviderError> {
        ...
        loop {
            ...
            if n == 0 {
                return Err(ProviderError::Network("connection closed unexpectedly".to_string()));
            }
    ...
    async fn fetch_all_albums_internal(&self) -> Result<Vec<ProviderAlbum>, ProviderError> {
        ...
        self.send_command(&mut reader, "list album group artist").await?;
        let response = self.read_response(&mut reader).await?;
        ...
        for (album_name, artist) in album_entries {
            ...
            self.send_command(&mut reader, &format!("find album \"{}\"", escape_mpd_string(&album_name))).await?;
            ...
            let year = tracks_response.lines()
                .find_map(|line| {
                    let line = line.trim();
    ...
    fn map_to_provider_track(&self, mpd_track: &HashMap<String, String>) -> Option<ProviderTrack> {
        ...
        let duration_secs = mpd_track.get("Time")
            .and_then(|t| t.parse::<f64>().ok());

        let track_number = mpd_track.get("Track")
            .and_then(|t| t.parse::<u32>().ok());

        let disc_number = mpd_track.get("Disc")

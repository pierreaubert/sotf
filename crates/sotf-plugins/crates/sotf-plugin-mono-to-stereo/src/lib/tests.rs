...
use super::mono_to_stereo_plugin::MonoToStereoPlugin;
    ...
    fn test_set_parameter_stereo_width() {
        let mut p = MonoToStereoPlugin::new();
        p.initialize(48000).unwrap();
        p.set_parameter(ParameterId::from("stereo_width"), ParameterValue::Float(0.75))
            .unwrap();
    ...
    fn test_set_parameter_haas_delay_ms_updates_samples() {
        let mut p = MonoToStereoPlugin::new();
        p.initialize(48000).unwrap();
        p.set_parameter(ParameterId::from("haas_delay_ms"), ParameterValue::Float(3.0))
            .unwrap();
    ...
    fn test_set_parameter_decor_frequencies_update_curve() {
        ...
        let mut p = MonoToStereoPlugin::new();
        p.initialize(48000).unwrap();
        p.set_parameter(ParameterId::from("decor_low_hz"), ParameterValue::Float(200.0))
            .unwrap();
        p.set_parameter(ParameterId::from("decor_high_hz"), ParameterValue::Float(1000.0))
            .unwrap();
        ...
        for i in 0..=low_bin {
            assert!(p.freq_width_curve[i].abs() < 1e-6, "bin {i} should be below low cutoff");
        }
    ...
    fn test_set_parameter_freq_dependent() {
        let mut p = MonoToStereoPlugin::new();
        p.initialize(48000).unwrap();
        p.set_parameter(ParameterId::from("freq_dependent"), ParameterValue::Bool(false))
            .unwrap();
        assert!(!p.freq_dependent);
        p.set_parameter(ParameterId::from("freq_dependent"), ParameterValue::Bool(true))
            .unwrap();
        assert!(p.freq_dependent);
    }
    ...
    fn test_set_parameter_out_of_range_clamps() {
        let mut p = MonoToStereoPlugin::new();
        p.initialize(48000).unwrap();
        p.set_parameter(ParameterId::from("stereo_width"), ParameterValue::Float(2.0))
            .unwrap();
        assert!(
            (p.stereo_width.target() - 1.0).abs() < 1e-6,
            "stereo_width should clamp to max 1.0"
        );
        p.set_parameter(ParameterId::from("stereo_width"), ParameterValue::Float(-1.0))
            .unwrap();
        assert!(
    ...
    fn test_process_zero_frames_returns_ok() {
        ...
        let produced = p.process(&input, &mut output, &ProcessContext::new(48000, 0))
            .unwrap();
    ...
    fn test_generate_decorrelation_filter_respects_low_hz() {
        ...
        let mut p = MonoToStereoPlugin::new();
        p.initialize(48000).unwrap();
        p.set_parameter(ParameterId::from("decor_low_hz"), ParameterValue::Float(500.0))
            .unwrap();
        ...
        for i in 1..low_bin {
            ...
            assert!(c.im.abs() < 1e-6, "bin {i} imag should be zero below low_hz");
            assert!(
                (c.re - 1.0).abs() < 1e-6,

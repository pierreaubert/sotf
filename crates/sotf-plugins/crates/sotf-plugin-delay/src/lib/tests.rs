...
use super::types::DelayPluginParams;
    ...
    fn test_process_in_place_impulse_known_delay() {
        ...
        assert!(buffer[0].abs() < 0.01, "frame 0 should be ~0, got {}", buffer[0]);
        ...
        assert!(buffer[peak_idx] > 0.9, "delayed impulse should be close to 1.0");
    }
    ...
    fn test_set_parameter_smoke_known_values() {
        ...
        p.set_parameter(ParameterId::from("lfo_depth_ms"), ParameterValue::Float(1.25))
            .unwrap();
        ...
        p.set_parameter(ParameterId::from("allpass_coeff"), ParameterValue::Float(0.7))
            .unwrap();
    ...
    fn test_set_parameter_rejects_non_finite() {
        ...
        assert!(
            p.set_parameter(ParameterId::from("delay_ms"), ParameterValue::Float(f32::NAN))
                .is_err()
        );
        assert!(
            p.set_parameter(

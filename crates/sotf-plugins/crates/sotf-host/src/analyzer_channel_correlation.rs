// ============================================================================
// Channel Correlation Analyzer Plugin
// ============================================================================
//
// Computes a running inter-channel Pearson correlation matrix over a sliding
// time window. Designed to feed the spatial spider visualizer in app-gpui so
// the user can see at a glance how strongly each pair of channels covaries
// (e.g. front-pair correlation collapsing under XTC, or rear channels
// correlating into a phantom centre under the upmixer).
//
// The matrix is stored row-major in `Arc<Vec<f32>>` of length `n*n`, where
// `matrix[ref * n + ch]` is the Pearson r between channels `ref` and `ch`.
...
#[inline]
fn upper_tri_index(i: usize, j: usize, n: usize) -> usize {
...
impl ChannelCorrelationMonitor {
    ...
    fn accumulate_aligned_frames(&mut self, body: &[f32]) {
        let n = self.channels;
        let num_frames = body.len() / n;
        if num_frames == 0 {
            return;
    ...
    #[inline]
    fn decay_state(&mut self, num_frames: usize) {
...
pub struct ChannelCorrelationPlugin {
    ...
    cache: RealTimeCache<CorrelationData>,
    monitor: ChannelCorrelationMonitor,

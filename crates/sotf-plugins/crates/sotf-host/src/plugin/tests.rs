...
use super::Plugin;
use super::loop_range;
use super::time_signature;
...
fn validate_parameter_rejects_nan() {
    let plugin = InPlacePluginAdapter::new(ValidatingPlugin);
    let err = plugin
        .validate_parameter(&ParameterId::from("gain_db"), &ParameterValue::Float(f32::NAN))
        .unwrap_err();
...
fn validate_parameter_rejects_unknown_id() {
    let plugin = InPlacePluginAdapter::new(ValidatingPlugin);
    let err = plugin
        .validate_parameter(&ParameterId::from("not_a_param"), &ParameterValue::Float(0.0))
        .unwrap_err();
...
fn validate_parameter_error_includes_parameter_id() {
    ...
    let err = plugin
        .validate_parameter(&ParameterId::from("gain_db"), &ParameterValue::Float(f32::NAN))
        .unwrap_err();
    assert!(err.contains("gain_db"), "error should name parameter: {}", err);
}

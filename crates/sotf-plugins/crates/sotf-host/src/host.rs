...
fn ensure_len_f64(buffer: &mut Vec<f64>, len: usize) {
    if buffer.len() < len {
        buffer.resize(len, 0.0);
    }
}
...
struct ProcessBuffers {
    ...
    compensation_delays: CompensationDelays,
...
struct CompensationDelays {
    #[allow(dead_code)]
    edge_keys: Vec<(NodeId, NodeId)>,
    delays: Vec<Option<LookaheadBuffer>>,
}

impl CompensationDelays {
    ...
    fn set(&mut self, edge_id: usize, delay: LookaheadBuffer) {
        if edge_id < self.delays.len() {
            self.delays[edge_id] = Some(delay);
        }
    }

    fn get_mut_edge(&mut self, edge_id: usize) -> Option<&mut LookaheadBuffer> {
    ...
    #[cfg(test)]
    fn get(&self, key: &(NodeId, NodeId)) -> Option<&LookaheadBuffer> {
        let idx = self
            .edge_keys
            .iter()
            .position(|candidate| candidate == key)?;
...
///
/// Move this to the control/UI thread and queue graph changes there; `DawHost`
/// applies them on the audio side before the next block is built or processed.
...
impl GraphMutationSender {
    ...
    /// Queue a plugin append for the linear chain host API.
    pub fn queue_add_plugin(&mut self, plugin: Box<dyn Plugin>) -> Result<(), String> {
        self.push_mutation(GraphMutation::AddPlugin { plugin })
    }
...
impl DawHost {
    pub fn new(channels: usize, sample_rate: u32) -> Self {
        ...
        Self {
            ...
            parameter_event_rx,
            parameter_event_scratch: Vec::new(),
    ...
    pub fn queue_add_plugin(&mut self, plugin: Box<dyn Plugin>) -> Result<(), String> {
        self.queue_graph_mutation(GraphMutation::AddPlugin { plugin })
    }
    ...
    fn apply_graph_mutation(&mut self, mutation: GraphMutation) -> Result<(), String> {
        match mutation {
            ...
            GraphMutation::AddPlugin { plugin } => self.add_plugin(plugin).map(|_| ()),
    ...
    ///
    /// The current graph buffer cache is f32-native, so this path keeps the API
    /// available without per-call heap allocation by reusing conversion
    /// scratch. Individual plugins can still expose true f64 processing through
    /// `Plugin::process_f64` for direct hosts and future graph-native f64 buffers.
    ...
    fn can_split_parameter_event_block_f64(&self, input: &[f64], output: &[f64]) -> bool {
        if !self.automation.is_empty()
            || !self.cached_frames_identity
            || !self.cached_rate_identity
            || !self.can_process_f64_chain_native()
        {
            return false;
        }
        let input_channels = self.input_channels();
    ...
    fn process_f64_split_parameter_events(
        ...
        while frame_cursor < frames {
            ...
            if next_event_frame > frame_cursor {
                ...
                let out_start = frame_cursor * output_channels;
                let out_end = next_event_frame * output_channels;
                let segment_frames = self.process_f64_chain_native(
                    &input[in_start..in_end],
                    &mut output[out_start..out_end],
                )?;
                processed_frames += segment_frames;
    ...
    fn process_f64_without_parameter_events(
        ...
        let result = self.process(&input_scratch[..in_len], &mut output_scratch[..out_len]);
    ...
    fn process_f64_chain_native(
        ...
        ensure_len_f64(&mut scratch_a, input.len());
        ...
        for idx in 0..self.chain_nodes.len() {
            ...
            let frames = if is_last {
                ...
                ensure_len_f64(&mut scratch_b, output_len);
                ...
                ensure_len_f64(&mut scratch_a, output_len);
                let frames = Self::process_f64_node(
                    self.plugins[nid].as_mut().unwrap().as_mut(),
                    self.bypassed.get(nid).copied().unwrap_or(false),
                    &scratch_b[..current_len],
    ...
    fn process_stage_parallel(
        ...
        while bufs.parallel_scratch.len() < plugins.len() {
            bufs.parallel_scratch
            ...
            .map(|&nid| {
                ...
                unsafe {
                    ...
                    let node_buffer_slot =
                        &mut *((node_buffers_addr as *mut Option<NodeBuffer>).add(nid));
                    ...
                    let in_len = if is_input_node.get(nid).copied().unwrap_or(false) {
                        ...
                        let source = (&*((node_buffers_addr as *const Option<NodeBuffer>)
                            .add(edge.from_node)))
                            .as_ref()
                            .ok_or_else(|| {
    ...
    fn merge_inputs_into(
        ...
        compensation_delays: &mut CompensationDelays,
    ) -> Result<usize, String> {
    ...
    fn sum_interleaved_at(
        ...
        if dest_offset == 0
            ...
            && dest.len() >= total
        {
            crate::simd::scale_add_simd(&mut dest[..total], &src_data[..total], 1.0);
            return;
        }
    ...
    fn collect_output_from_buffers(
        ...
        if ons.len() == 1 {
            ...
            for &id in ons {
                ...
                let l = d.len().min(out.len());
                crate::simd::scale_add_simd(&mut out[..l], &d[..l], 1.0);
            }
        }
        Ok(())
    }
    ...
    /// at merge points.
    fn compute_compensation_delays(&mut self, num_slots: usize) -> CompensationDelays {
        ...
        let mut delays = CompensationDelays::new(&self.edges);
            ...
            for &nid in &stage.nodes {
                ...
                for edge in preds {
                    ...
                    if compensation > 0 {
                        ...
                        delays.set(edge.id, LookaheadBuffer::new(compensation, delay_channels));

...
public class BandMergeViewController: AUViewController, AUAudioUnitFactory {
    nonisolated(unsafe) private var audioUnit: BandMergeAudioUnit?
    nonisolated(unsafe) private var gpuiView: GPUIAUView?

    public nonisolated func createAudioUnit(with componentDescription: AudioComponentDescription) throws -> AUAudioUnit {
        let unit = try BandMergeAudioUnit(componentDescription: componentDescription, options: [])
        self.audioUnit = unit
        return unit
    }
    ...
    public override func viewDidLoad() {
        super.viewDidLoad()
        let gpui = GPUIAUView(pluginType: "BandMerge", audioUnit: audioUnit)

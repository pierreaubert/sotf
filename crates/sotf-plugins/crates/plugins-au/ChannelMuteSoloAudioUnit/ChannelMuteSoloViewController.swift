...
public class ChannelMuteSoloViewController: AUViewController, AUAudioUnitFactory {
    nonisolated(unsafe) private var audioUnit: ChannelMuteSoloAudioUnit?
    nonisolated(unsafe) private var gpuiView: GPUIAUView?

    public nonisolated func createAudioUnit(with componentDescription: AudioComponentDescription) throws -> AUAudioUnit {
        let unit = try ChannelMuteSoloAudioUnit(componentDescription: componentDescription, options: [])
        self.audioUnit = unit
        return unit
    }
    ...
    public override func viewDidLoad() {
        super.viewDidLoad()
        let gpui = GPUIAUView(pluginType: "ChannelMuteSolo", audioUnit: audioUnit)

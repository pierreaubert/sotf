...
public class DenoiserViewController: AUViewController, AUAudioUnitFactory {
    nonisolated(unsafe) private var audioUnit: DenoiserAudioUnit?
    nonisolated(unsafe) private var gpuiView: GPUIAUView?

    public nonisolated func createAudioUnit(with componentDescription: AudioComponentDescription) throws -> AUAudioUnit {
        let unit = try DenoiserAudioUnit(componentDescription: componentDescription, options: [])
        self.audioUnit = unit
        return unit
    }
    ...
    public override func viewDidLoad() {
        super.viewDidLoad()
        let gpui = GPUIAUView(pluginType: "Denoiser", audioUnit: audioUnit)

...
use math_audio_iir_fir :: { Biquad , KautzFilter , WarpedBiquad } ;
...
#[test]
    ...
    fn test_set_parameter_out_of_range_band_index_does_not_panic() {
        ...
        assert!(result.is_ok(), "out-of-range band should not panic/error: {:?}", result);
    }
    ...
    fn test_svf_topology_processes_finite_output() {
        ...
        assert!(buffer.iter().all(|s| s.is_finite()), "SVF output must be finite");
        // With a non-silent input, some sample should differ from input after filtering.
        let input_energy: f32 = (0..num_frames * 2).map(|i| ((i as f32 * 0.05).sin() * 0.5).powi(2)).sum();
        ...
        assert!((output_energy - input_energy).abs() > 1e-6, "SVF should change signal energy");
    }

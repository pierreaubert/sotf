// ============================================================================
// Property-Based Tests for sotf-plugin-fir-designer
// ============================================================================
...
proptest! {
    ...
    fn process_finite_output_default(buffer in mono_buffer_strategy()) {
        ...
        prop_assert!(buf.iter().all(|s| s.is_finite()),
            "Default FIR Designer should produce finite output");
    }
    ...
    fn process_finite_output_with_active_band(buffer in mono_buffer_strategy()) {
        ...
        let mut plugin = FirDesignerPlugin::from_params(1, 48000, params).unwrap();
        let mut buf = buffer.clone();
        plugin.process_in_place(&mut buf, &ProcessContext::new(48000, 64)).unwrap();

        prop_assert!(buf.iter().all(|s| s.is_finite()),
            "Active FIR Designer band should produce finite output");
    }
    ...
    fn roundtrip_band_active(enabled in prop::bool::ANY) {
        ...
        prop_assert_eq!(got, Some(ParameterValue::Bool(enabled)),
            "band_0_active set->get should round-trip");
    }
    ...
    // -------------------------------------------------------------------------
    // Latency: linear-phase mode should report correct group delay
    // -------------------------------------------------------------------------
    #[test]
    fn linear_phase_latency_matches_fir_length(
        ...
        let params = FirDesignerPluginParams {
            ...
            phase_mode_index: 0, // Linear phase
            auto_gain: false,
            mix: 1.0,
            filters: vec![BandConfig {
                filter_type: "Peak".to_string(),
    ...
    fn minimum_phase_latency_is_zero(
        ...
        let params = FirDesignerPluginParams {
            num_filters: 1,
            fir_length_index: 0,
            phase_mode_index: 1, // Minimum phase
            auto_gain: false,
            mix: 1.0,
            filters: vec![BandConfig {
                filter_type: "Peak".to_string(),
        ...
        prop_assert_eq!(plugin.latency_samples(), 0,
            "Minimum-phase latency should be zero");
    }
}

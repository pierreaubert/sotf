...
use super::fir_designer_plugin::FirDesignerPlugin;
use super::misc::{
...
fn test_large_block_ola_matches_small_chunks() {
    ...
    let mut one_block = FirDesignerPlugin::from_params(channels, sr, params.clone()).unwrap();
    let mut chunked = FirDesignerPlugin::from_params(channels, sr, params).unwrap();
...
fn test_overlap_buffers_match_fir_tail_length() {
    let channels = 3;
    let sr = 48000;
    let mut plugin = FirDesignerPlugin::new(channels, sr);
    ...
    plugin
        .parametric_set_parameter(ParameterId::from("fir_length"), ParameterValue::Int(3))
        .unwrap();
    let expected_tail = plugin.fir_length() - 1;
    assert!(expected_tail < plugin.fft_size);
    for overlap in &plugin.overlap {
...
fn test_rebuild_fir_reuses_design_scratch_vectors() {
    ...
    plugin
        .parametric_set_parameter(ParameterId::from("band_0_gain"), ParameterValue::Float(6.0))
        .unwrap();
    plugin.rebuild_fir();
...
#[test]
fn test_fir_designer_latency() {
    let plugin = FirDesignerPlugin::new(2, 48000);
...
fn test_dc_gain_not_hardcoded() {
    ...
    let plugin = FirDesignerPlugin::from_params(channels, sr, params).unwrap();
...
fn test_set_parameter_num_filters_grows_bands() {
    ...
    plugin
        .parametric_set_parameter(ParameterId::from("num_filters"), ParameterValue::Int(8))
        .unwrap();
...
fn test_reset_clears_overlap() {
    let mut plugin = FirDesignerPlugin::new(2, 48000);
    // Manually seed overlap so we can verify reset clears it.
...
fn test_process_zero_channels() {
    let mut plugin = FirDesignerPlugin::new(0, 48000);
...
fn test_initialize_changes_sample_rate() {
    let mut plugin = FirDesignerPlugin::new(1, 44100);
...
fn test_from_params_fills_missing_bands() {
    ...
    let plugin = FirDesignerPlugin::from_params(1, 48000, params).unwrap();
    assert_eq!(plugin.bands.len(), 3);
    assert_eq!(plugin.bands[0].frequency, 500.0);
    assert_eq!(plugin.bands[1].frequency, 1000.0); // default fill
}

#[test]
fn test_info_and_channels() {
    let plugin = FirDesignerPlugin::new(4, 48000);
...
fn test_parameters_reflect_state() {
    ...
    assert!(before >= 5);
    // Verify the name of a per-band parameter is present.
    assert!(
        plugin
            .parameters()
            .iter()
...
fn test_band_contribution_db_skips_inactive() {
    ...
    let with_active = FirDesignerPlugin::band_contribution_db(&[active_band], 1000.0);
    let with_inactive = FirDesignerPlugin::band_contribution_db(&[inactive_band], 1000.0);
    assert!(with_active > 1.0);
    assert!(with_inactive.abs() < 0.01);
}

#[test]
fn test_minimum_phase_latency() {
    let mut plugin = FirDesignerPlugin::new(1, 48000);
...
fn test_set_parameter_band_type() {
    ...
    plugin
        .parametric_set_parameter(ParameterId::from("band_0_type"), ParameterValue::Int(3))
        .unwrap();
...
fn test_get_parameter_band_type_q_active() {
    ...
    let t = plugin
        .parametric_get_parameter(&ParameterId::from("band_0_type"))
        .unwrap();
    assert_eq!(
        t,
    ...
    let q = plugin
        .parametric_get_parameter(&ParameterId::from("band_0_q"))
        .unwrap();
    assert_eq!(q, ParameterValue::Float(2.5));

    let active = plugin
        .parametric_get_parameter(&ParameterId::from("band_0_active"))
        .unwrap();
...
fn test_set_parameter_auto_gain_non_bool_rejected() {
    ...
    let result =
        plugin.parametric_set_parameter(ParameterId::from("auto_gain"), ParameterValue::Float(1.0));

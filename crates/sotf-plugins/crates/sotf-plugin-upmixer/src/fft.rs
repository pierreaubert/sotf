...
impl UpmixerPlugin {
    ...
    pub(super) fn apply_window_and_forward_fft(&mut self, input: &[f32]) {
        ...
        sotf_host::simd::window_mul_simd_inplace(&mut self.time_domain_right, &self.window);

        if headroom_scale != 1.0 {
            sotf_host::simd::scale_add_simd_inplace(&mut self.time_domain_left, headroom_scale);
            sotf_host::simd::scale_add_simd_inplace(&mut self.time_domain_right, headroom_scale);
        }

...
pub struct DenoiserPlugin {
    ...
    pub(super) spectrum_size: usize, // fft_size / 2 + 1
    ...
    pub(super) prev_power: Vec<Vec<f32>>, // [channels][spectrum_size] previous frame power

    // Psychoacoustic masking
    pub(super) psychoacoustic_masking: bool,
    pub(super) bark_map: Vec<f32>, // [spectrum_size] frequency-to-Bark mapping
    pub(super) bark_bin_range: Vec<(usize, usize)>, // [spectrum_size] precomputed (lo, hi) bin range within MAX_SPREAD_BARK
    pub(super) masking_threshold: Vec<f32>, // [spectrum_size] scratch for masking thresholds
    pub(super) masking_signal_power: Vec<f32>, // [spectrum_size] scratch for signal power

    // Noise profile capture
    pub(super) use_captured_profile: bool,
    pub(super) has_noise_profile: bool,
    pub(super) noise_profile_storage: Vec<Vec<f32>>, // [channels][spectrum_size] pre-allocated
    pub(super) learning_accumulator: Vec<Vec<f32>>,  // [channels][spectrum_size]
    pub(super) learning_frames_count: usize,
    pub(super) learning_frames_target: usize,
    pub(super) is_learning: bool,

    // Pre-computed coefficients
    pub(super) attack_coeff: f32,
    pub(super) release_coeff: f32,
    pub(super) reduction_linear: f32,
    pub(super) floor_linear: f32,
    ...
    pub(super) time_domain: Vec<Vec<f32>>, // [channels][fft_size]
    pub(super) freq_domain: Vec<Vec<Complex<f32>>>, // [channels][spectrum_size]

    // MCRA state (per-channel, per-bin)
    pub(super) noise_psd: Vec<Vec<f32>>, // Estimated noise power spectrum
    pub(super) smoothed_psd: Vec<Vec<f32>>, // Smoothed signal PSD (S_tmp)
    pub(super) min_psd: Vec<Vec<f32>>,   // Minimum PSD tracker — window A
    pub(super) min_psd_b: Vec<Vec<f32>>, // Minimum PSD tracker — window B (IMCRA)
    pub(super) speech_presence: Vec<Vec<f32>>, // Speech presence probability (p)
    pub(super) frame_counter: Vec<usize>, // Per-channel frame count

    // Wiener filter state
    pub(super) gain: Vec<Vec<f32>>, // Current Wiener gains per bin
    pub(super) smoothed_gain: Vec<Vec<f32>>, // Temporally smoothed gains
    // Spatial denoising state
    pub(super) spatial_coherence: Vec<f32>, // Smoothed MSC coherence estimate per bin
    pub(super) spatial_cross: Vec<Complex<f32>>, // Averaged complex cross-spectrum estimate per bin

    // Frequency smoothing scratch buffer and precomputed kernel
    pub(super) freq_smooth_temp: Vec<f32>, // [spectrum_size] scratch for smoothing across bins
    pub(super) freq_smooth_kernel: (f32, f32, f32), // Precomputed (c0, c1, c2) Gaussian weights

    // Overlap-add buffers
    pub(super) input_buffer: Vec<f32>, // Interleaved input accumulator
    pub(super) input_buffer_fill: usize,
    pub(super) temp_input_block: Vec<f32>, // Pre-allocated block for FFT input

    // Ring-buffer output accumulator (per-channel, power-of-2 capacity)
    pub(super) output_accumulator: Vec<Vec<f32>>, // [channels][ring_capacity]
    pub(super) output_ring_mask: usize,           // ring_capacity - 1 (for & masking)
    pub(super) output_read_pos: usize,            // read position in ring
    pub(super) output_write_pos: usize,           // next overlap-add write position
    pub(super) output_accumulator_fill: usize,    // frames available for reading
    ...
    pub(super) pnd_analyzers: Vec<PndAnalyzer>,

    // Formant preservation
    pub(super) formant_preserver: super::wiener::FormantPreserver,

    // Multi-resolution dual-STFT processing
    pub(super) multi_resolution: bool,
    ...
    pub(super) multi_res_state: Option<super::multi_resolution::MultiResState>,

    // --- Phase 4B: SOTA additions ---
    pub(super) harmonic_percussive: bool,
    pub(super) spatial_denoise: bool,
    pub(super) spatial_strength: f32,
    /// Per-channel tonal/transient separator for harmonic/percussive mode
    pub(super) tonal_transient_seps: Vec<math_audio_dsp::tonal_transient::TonalTransientSeparator>,
    /// Scratch buffers for tonal/transient masks [spectrum_size]
    pub(super) tt_magnitudes: Vec<f32>,
    pub(super) tt_tonal_mask: Vec<f32>,
    pub(super) tt_transient_mask: Vec<f32>,

    // Data exposure for UI — cached to avoid allocations in get_data()
    pub(super) avg_reduction_db: f32,
    pub(super) learning_active: bool,
    pub(super) cache: RealTimeCache<DenoiserData>,
    pub(super) data_update_counter: usize,
    pub(super) cached_noise_floor_buf: Vec<f32>, // [NUM_DISPLAY_BANDS] pre-allocated
    pub(super) cached_snr_buf: Vec<f32>,         // [NUM_DISPLAY_BANDS] pre-allocated
...
impl DenoiserPlugin {
    ...
    pub fn from_params(channels: usize, params: DenoiserPluginParams) -> Self {
        ...
        if plugin.multi_resolution {
            plugin.multi_res_state = Some(super::multi_resolution::MultiResState::new(
                channels,
                plugin.mcra_alpha_s,
                plugin.mcra_alpha_p,
                plugin.mcra_l,
        ...
        match idx {
            ...
            25 => {
                ...
                if self.multi_resolution && self.multi_res_state.is_none() {
                    self.multi_res_state = Some(super::multi_resolution::MultiResState::new(
                        self.channels,
                        self.mcra_alpha_s,
                        self.mcra_alpha_p,
                        self.mcra_l,

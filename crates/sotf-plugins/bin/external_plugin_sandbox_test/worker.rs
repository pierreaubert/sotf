...
use serde_json::json;
use sotf_plugins :: { ExternalPluginProcessEvent , IsolatedExternalPluginWorkerReport , PluginHost , PluginSandboxStatusCode } ;
...
pub(super) fn worker_reports_json(reports: &[IsolatedExternalPluginWorkerReport]) -> Vec<serde_json::Value> {
    reports
        .iter()
        .map(|report| {
            json!({

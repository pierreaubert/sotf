...
use super::security :: { KeyManager , PeerClass , classify_peer , current_uid as security_current_uid , ensure_secure_socket_dir , peer_allows_command , verify_peer_credentials } ;
...
impl AudioDaemon {
    ...
    pub(super) async fn handle_add_plugin(&self, plugin: PluginConfig, index: Option<usize>) -> Response {
    ...
    pub(super) async fn reload_plugins_with_user_plugins(&self, plugins: Vec<PluginConfig>) -> Response {
        let prepared_plan = {
            let pipeline = self.system_state.lock();
    ...
    pub(super) fn sync_encryption_to_shared_memory(&self, _flush_audio: bool) -> Result<(), String> {
        Ok(())
    }

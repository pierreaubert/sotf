...
use head_scanner::{
    camera::Camera,
    reconstruction::{
        estimate_essential_matrix, recover_pose_from_essential, triangulate_point,
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    println!("   ✓ Camera intrinsics: fx={}, fy={}", intrinsics.fx, intrinsics.fy);
    ...
    println!("   Matched {} feature pairs (Lowe's ratio test)", matches.len());
            ...
            match recover_pose_from_essential(&essential, &points1, &points2, &intrinsics, &inliers) {
                Ok(pose) => {
                            ...
                            match triangulate_point(&pt1, &pt2, &pose1, &pose, &intrinsics) {
                                Ok(point_3d) => {
                                    ...
                                    if triangulated_count <= 3 {
                                        println!("   Point {}: ({:.2}, {:.2}, {:.2}) depth={:.2}cm", 
                                            triangulated_count, 
                    ...
                    if triangulated_count > 0 {
                        ...
                        println!("   Average depth: {:.2}cm", total_depth / triangulated_count as f32);
                    }
                }
                Err(e) => {

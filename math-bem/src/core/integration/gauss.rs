...
pub fn gauss_legendre(order: usize) -> (Vec<f64>, Vec<f64>) {
    assert!(order >= 1 && order <= N_GAUORDER, "Order must be 1..{}", N_GAUORDER);
...
fn gauss_legendre_raw(n: usize) -> (&'static [f64], &'static [f64]) {
    match n {
        ...
        _ => {
            ...
            if n <= 2 { (&GL2_X, &GL2_W) }
            else if n <= 4 { (&GL4_X, &GL4_W) }
            else if n <= 6 { (&GL6_X, &GL6_W) }
            else if n <= 8 { (&GL8_X, &GL8_W) }
            else if n <= 12 { (&GL12_X, &GL12_W) }
            else if n <= 16 { (&GL16_X, &GL16_W) }
            else { (&GL20_X, &GL20_W) }
        }
    }
}
...
pub fn triangle_quadrature(order: usize) -> Vec<(f64, f64, f64)> {
    ...
    match order {
        1 => GAUCORWEI_TR1.iter().map(|&[x, y, w]| (x, y, w * AREA_SCALE)).collect(),
        2 => GAUCORWEI_TR4.iter().map(|&[x, y, w]| (x, y, w * AREA_SCALE)).collect(),
        3 => GAUCORWEI_TR7.iter().map(|&[x, y, w]| (x, y, w * AREA_SCALE)).collect(),
        _ => GAUCORWEI_TR13.iter().map(|&[x, y, w]| (x, y, w * AREA_SCALE)).collect(),
    }
}

...
fn main() {
    ...
    for ka_target in [0.2, 0.5, 1.0, 2.0] {
        ...
        println!("\n============================================================");
        println!("=== ka = {:.2} (f = {:.1} Hz, k = {:.4}) ===", ka, frequency, k);
        println!("============================================================");
...
fn investigate_incident_field(
    ...
    let dpdn_min = dpdn_inc.iter().map(|x| x.norm()).fold(f64::INFINITY, f64::min);
    ...
    println!("  Incident pressure |p_inc|: min={:.6}, max={:.6}", p_inc_min, p_inc_max);
    println!("  Normal derivative |dp/dn|: min={:.6}, max={:.6}", dpdn_min, dpdn_max);
    ...
    for (i, elem) in mesh.elements.iter().enumerate() {
        let dist = ((elem.center[0]).powi(2) + (elem.center[1]).powi(2)
    ...
    println!("    p_inc = {:.6} + {:.6}i", p_inc[forward_idx].re, p_inc[forward_idx].im);
    println!("    dp/dn = {:.6} + {:.6}i", dpdn_inc[forward_idx].re, dpdn_inc[forward_idx].im);
    println!("    rhs_bm = {:.6} + {:.6}i", rhs_bm[forward_idx].re, rhs_bm[forward_idx].im);
    ...
    println!("\n  Expected (analytical):");
    println!("    p_inc ≈ exp(ik*r) = cos({:.4}) = {:.6}", k * radius, expected_p);
...
fn investigate_system_matrix(
    ...
    let diag_min = diag_magnitudes.iter().cloned().fold(f64::INFINITY, f64::min);
    ...
    let off_diag_avg: f64 = off_diag_magnitudes.iter().sum::<f64>() / off_diag_magnitudes.len() as f64;

    println!("  Matrix size: {} x {}", n, n);
    println!("  Diagonal |A[i,i]|: min={:.6}, avg={:.6}, max={:.6}", diag_min, diag_avg, diag_max);
    println!("  Off-diagonal: avg={:.6}, max={:.6}", off_diag_avg, off_diag_max);
    ...
    println!("    beta = {:.6} + {:.6}i (i.e., i/k where k={:.4})", beta.re, beta.im, physics.wave_number);
    println!("    tau = {:.6}", tau);
    ...
    for i in 0..5.min(n) {
        ...
        println!("    A[{},{}] = {:.6} + {:.6}i (|.| = {:.6})", i, i, d.re, d.im, d.norm());
    }
    ...
    println!("      Total E        = {:.6} + {:.6}i", result.d2g_dnxdny_integral.re, result.d2g_dnxdny_integral.im);
    println!("      k²*G*(n·n)     = {:.6} + {:.6}i (should dominate at high k)", k2_g_contribution.re, k2_g_contribution.im);
    println!("      Edge contrib   = {:.6} + {:.6}i (E - k²*G)", edge_e_estimate.re, edge_e_estimate.im);
    println!("      Ratio |edge|/|k²G| = {:.2}", edge_e_estimate.norm() / k2_g_contribution.norm().max(1e-15));

    println!("    G integral   = {:.6} + {:.6}i (|.| = {:.6})",
        result.g_integral.re, result.g_integral.im, result.g_integral.norm());
    println!("    H integral   = {:.6} + {:.6}i (|.| = {:.6})",
        result.dg_dn_integral.re, result.dg_dn_integral.im, result.dg_dn_integral.norm());
    println!("    H^T integral = {:.6} + {:.6}i (|.| = {:.6})",
        result.dg_dnx_integral.re, result.dg_dnx_integral.im, result.dg_dnx_integral.norm());
    println!("    E integral   = {:.6} + {:.6}i (|.| = {:.6})",
        result.d2g_dnxdny_integral.re, result.d2g_dnxdny_integral.im, result.d2g_dnxdny_integral.norm());
    ...
    let coeff_from_integrals = result.dg_dn_integral * gamma_c * tau_c + result.d2g_dnxdny_integral * beta;
    let free_term = Complex64::new(-gamma * 0.5, 0.0);
    let total_diag = free_term + coeff_from_integrals;

    println!("\n    Computed diagonal contribution:");
    println!("      Free term (-γ/2)     = {:.6} + {:.6}i", free_term.re, free_term.im);
    println!("      γτH                  = {:.6} + {:.6}i",
    ...
    println!("      Total (free + γτH + βE) = {:.6} + {:.6}i", total_diag.re, total_diag.im);
    println!("      Actual A[0,0]        = {:.6} + {:.6}i", system.matrix[[0, 0]].re, system.matrix[[0, 0]].im);

    // Scaling analysis
    let k = physics.wave_number;
    println!("\n    Scaling analysis (k = {:.4}):", k);
    println!("      |G| / (1/k)    = {:.6} (expect O(1))", result.g_integral.norm() * k);
    println!("      |H| / k        = {:.6} (expect O(1) for curved surface)", result.dg_dn_integral.norm() / k);
    println!("      |E| / k²       = {:.6} (expect O(1))", result.d2g_dnxdny_integral.norm() / (k * k));
    println!("      |βE|           = {:.6} (expect O(k))", (result.d2g_dnxdny_integral * beta).norm());
...
fn investigate_solution(
    ...
    let p_max_trad = solution_trad.x.iter().map(|x| x.norm()).fold(0.0f64, f64::max);
    let p_avg_trad: f64 = solution_trad.x.iter().map(|x| x.norm()).sum::<f64>() / n as f64;

    // Floored solution statistics
    let p_max_floored = solution_floored.x.iter().map(|x| x.norm()).fold(0.0f64, f64::max);
    let p_avg_floored: f64 = solution_floored.x.iter().map(|x| x.norm()).sum::<f64>() / n as f64;

    // CBIE-only solution statistics
    let p_max_cbie = solution_cbie.x.iter().map(|x| x.norm()).fold(0.0f64, f64::max);
    let p_avg_cbie: f64 = solution_cbie.x.iter().map(|x| x.norm()).sum::<f64>() / n as f64;

    println!("  CBIE-only (β=0, no hypersingular):");
    println!("    Surface |p|: avg={:.6}, max={:.6}", p_avg_cbie, p_max_cbie);
    println!("    Diagonal A[0,0] = {:.6} + {:.6}i",
        system_cbie.matrix[[0, 0]].re, system_cbie.matrix[[0, 0]].im);

    println!("\n  Traditional Burton-Miller β=i/k:");
    println!("    β = {:.6}i, |β*E| ≈ {:.1}", physics.burton_miller_beta().im, physics.burton_miller_beta().im * edge_e_magnitude);
    println!("    Surface |p|: avg={:.6}, max={:.6}", p_avg_trad, p_max_trad);
    println!("    Diagonal A[0,0] = {:.6} + {:.6}i",
        system_traditional.matrix[[0, 0]].re, system_traditional.matrix[[0, 0]].im);

    println!("\n  Floored β = max(i/k, i*{:.4}) to keep |β*E| ≥ {:.0}:", min_beta_e / edge_e_magnitude, min_beta_e);
    println!("    β = {:.6}i, |β*E| ≈ {:.1}", beta_floored.im, beta_floored.im * edge_e_magnitude);
    println!("    Surface |p|: avg={:.6}, max={:.6}", p_avg_floored, p_max_floored);
    println!("    Diagonal A[0,0] = {:.6} + {:.6}i",
        system_floored.matrix[[0, 0]].re, system_floored.matrix[[0, 0]].im);
    ...
    println!("\n  Analytical surface |p| at forward point: {:.6}", analytical_p_surface);

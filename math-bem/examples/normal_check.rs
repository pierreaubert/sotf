...
fn check_normals(mesh: &bem::core::types::Mesh, radius: f64, name: &str) {
    ...
    for (i, elem) in mesh.elements.iter().enumerate().take(5) {
        ...
        println!("           normal=({:.4},{:.4},{:.4})", elem.normal[0], elem.normal[1], elem.normal[2]);
        println!("           expected=({:.4},{:.4},{:.4})", expected_nx, expected_ny, expected_nz);
        println!("           dot={:.4} ({})", dot, if dot > 0.0 { "OUTWARD" } else { "INWARD" });
    }

    // Also check front (z>0) and back (z<0) normals
    println!("\n  Front elements (z > 0):");
    let front_elems: Vec<_> = mesh.elements.iter().filter(|e| e.center[2] > 0.05).take(3).collect();
    for elem in &front_elems {
        println!("    center z={:.4}, normal z={:.4}", elem.center[2], elem.normal[2]);
    }

    println!("  Back elements (z < 0):");
    let back_elems: Vec<_> = mesh.elements.iter().filter(|e| e.center[2] < -0.05).take(3).collect();

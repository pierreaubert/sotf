        ...
        for j in 0..m {
            ...
            if config.print_interval > 0 && total_iterations % config.print_interval == 0 {
                log::info!(
                    ...
                    total_iterations, restarts, rel_residual.to_f64().unwrap_or(0.0)
                );
            }
...
fn inner_product<T: ComplexField>(x: &Array1<T>, y: &Array1<T>) -> T {
    x.iter().zip(y.iter()).fold(T::zero(), |acc, (&xi, &yi)| acc + xi.conj() * yi)
}

#[inline]
fn vector_norm<T: ComplexField>(x: &Array1<T>) -> T::Real {
    x.iter().map(|xi| xi.norm_sqr()).fold(T::Real::zero(), |acc, val| acc + val).sqrt()
}

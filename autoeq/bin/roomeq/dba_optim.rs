...
pub fn optimize_dba(
    ...
    let objective_data = autoeq::workflow::setup_multisub_objective_data(&args, drivers_data.clone());
...
fn sum_array_response(sources: &[super::types::MeasurementSource]) -> Result<Curve, Box<dyn Error>> {
    if sources.is_empty() {

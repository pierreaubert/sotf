...
use sotf_audio::plugins::{Plugin, PluginConfig, ProcessContext};
...
pub extern "C" fn plugin_create(
    ...
    let result = panic::catch_unwind(|| {
        ...
        // Parse JSON config
        let plugin_config = match plugin_factory::create_plugin_config(plugin_type_str, config_str)
        {
            Ok(cfg) => cfg,
            Err(e) => {
...
pub extern "C" fn plugin_process(
    ...
    let result = Ok(unsafe { process_impl(handle, input, output, num_frames) });

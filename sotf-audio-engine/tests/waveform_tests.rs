...
use sotf_audio::waveform::{analyze_waveform, WAVEFORM_SAMPLES};
...
fn create_sine_wav(sample_rate: u32, channels: u16, duration_secs: f32, frequency: f32) -> NamedTempFile {
    let spec = WavSpec {
        channels,
        sample_rate,
        bits_per_sample: 16,
...
fn test_waveform_output_length() {
    ...
    assert!(result.is_ok(), "Waveform analysis failed: {:?}", result.err());
    
    let waveform = result.unwrap();
    assert_eq!(waveform.len(), WAVEFORM_SAMPLES, "Waveform should have exactly {} samples", WAVEFORM_SAMPLES);
}
...
fn test_waveform_values_in_range() {
    ...
    for (i, &value) in waveform.iter().enumerate() {
        ...
        assert!(value <= 255, "Waveform value at {} out of range: {}", i, value);
    }
}
...
fn test_waveform_deterministic() {
    ...
    assert_eq!(waveform1, waveform2, "Waveform analysis should be deterministic");
}

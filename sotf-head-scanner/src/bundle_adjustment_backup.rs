...
//!
//! - **Full Jacobian**: Complete 2x6 camera Jacobian with rotation derivatives
//! - **Sparse Solver**: Schur complement trick with sparse Cholesky decomposition
//! - **Rotation**: SO(3) Lie algebra for proper rotation updates
//! - **Optimization**: Levenberg-Marquardt with adaptive damping
//!
    ...
    fn compute_jacobian_and_residuals(
        ...
        for (point_idx, point_obs) in points_with_obs.iter().enumerate() {
            ...
            for (cam_idx, observed_2d) in &point_obs.observations {
                ...
                let (first, rest) = jacobian.split_at_mut(residual_idx + 1);
                self.compute_point_jacobian(
                    &mut first[residual_idx],
                    &mut rest[0],
                    point_param_offset,
                    &point_cam,
                );
